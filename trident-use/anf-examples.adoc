...
*The `ro` mount option is not supported for the Azure NetApp Files subvolume driver or any other driver. If you specify `ro` for the Azure NetApp Files subvolume driver, backend creation will fail. If you specify `ro` mount option for any other driver, CSI operations will fail.*
...
// Start snippet: collapsible block (open on page load)
.Example 2: Minimal configuration for Azure NetApp Files Subvolume
[%collapsible%open]
====
This is a basic Azure NetApp Files Subvolume backend. It uses two pre-created parent volumes named "parent-volume-1" and "parent-volume-2".
