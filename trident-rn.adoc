...
** Fixed a critical issue where incorrect iSCSI devices were discovered when detaching volumes from Kubernetes nodes.
** Fixed an issue where NQNs were not checked before they are unmapped from Subsystems.
** Fixed an issue where multiple attempts to close a LUKS device resulted in failures to detach volumes.
** Fixed iSCSI volume unstage when the device path has changed since its creation.
** Blocked cloning of volume across different storage classes.
* *Openshift*:
...
** Fixed an issue where iSCSI node prep failed with OCP 4.19.
* Increased timeout when cloning a volume using SolidFire backends (link:https://github.com/NetApp/trident/issues/1008[Issue #1008]).
