...
You can automates the force-detach process through integration with node health check (NHC) operator. For more information, see link:..trident-get-started/kubernetes-customize-deploy.html#details-about-force-detach[Details about force detach^].
|`false`
