...
== Customize security context constraints for Trident protect containers
...
|name
|Specifies the name of the SCC. | trident-protect-job

| priority
| Defines the priority of the SCC. SCCs with higher priority values are assessed before those with lower values. | 1
|===
+
. Apply the values from the `sccconfig.yaml` file using either the `helm install` or `helm upgrade` command:
+
*Install command*:
+
[source,console]
----
helm install trident-protect netapp-trident-protect/trident-protect -f sccconfig.yaml
----
+
*Upgrade command*:
+
...
----  
+
This will override the default values with the ones specified in the `sccconfig.yaml` file.
