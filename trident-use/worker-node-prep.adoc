...
[.lead]
All worker nodes in the Kubernetes cluster must be able to mount the volumes you have provisioned for your pods. If you are using the `ontap-nas`, `ontap-nas-economy`, or `ontap-nas-flexgroup` driver for one of your backends, your worker nodes require NFS tools. Otherwise they require iSCSI tools.

Recent versions of RedHat CoreOS have NFS and iSCSI installed by default.

WARNING: Reboot your worker nodes after installing the NFS or iSCSI tools to prevent failure when attaching volumes to containers.
...
== NFS volumes
Install the NFS tools if you are using the `ontap-nas`, `ontap-nas-economy`, or `ontap-nas-flexgroup` driver for one of your backends. Use the commands for your operating system to install the tools. 
...
----
====
NOTE: Ensure the NFS service is started up during boot time.
...
Astra Trident runs iSCSI self-healing every five minutes to:
...
* Stale or missing iSCSI sessions
* Missing LUNs
* A false sense of healthy pods
* Failures during node volume expansion

=== Considerations when using iSCSI volumes
