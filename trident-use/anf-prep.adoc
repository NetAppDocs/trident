...
NOTE: `subscriptionID`, `tenantID`, `clientID`, `location`, and `clientSecret` are optional when using managed identities on an AKS cluster. `tenantID`, `clientID`, `location`, and `clientSecret` are optional when using a cloud identity on an AKS cluster.
