...
NOTE: Restoring a backup to a namespace with existing resources will not alter any resources that share names with those in the backup. To restore all resources in a backup, either delete and re-create the target namespace, or restore the backup to a new namespace.
