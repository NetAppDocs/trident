...
|`cloudProvider` | Set to `"Azure"` when using managed identities or a cloud identity on an AKS cluster. Set to "AWS" when cloud identity on an EKS cluster. |`""` 

|`cloudIdentity` |Set to workload identity when using cloud identity on an AKS cluster. Set AWS IAM role when using cloud identity on an EKS cluster|`""` 
