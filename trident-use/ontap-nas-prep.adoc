...
+
Add the `trident.netapp.io/smbShareAdUser` annotation to the storage class to enable secure SMB.
...
You can choose one of the following for `smbShareAdUserPermission`: `full_control`, `change`, or `read`. The default permission is `full-control`. 
