...
[WARNING]
.Important notes for 22.10
===============================
* Kubernetes 1.25 is now supported in Trident. You should upgrade Astra Trident prior to upgrading Kubernetes.
...
** Added support for link:../trident-use/volume-share.html[cross-namespace volume sharing].
...
* Added link:../trident-reco/security-reco.html#use-linux-unified-key-setup-luks[support for LUKS-encrypted volumes] for `ontap-san` and `ontap-san-economy` storage drivers.

* Added support for Windows Server 2019 nodes.

* Added link:../trident-use/anf.html[support for SMB volumes on Windows nodes] through the `azure-netapp-files` storage driver.
