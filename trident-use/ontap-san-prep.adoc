...
NOTE: If using "admin" credentials, an aggregate need not be assigned to the SVM. While using “vsadmin” credentials with ASA r2, the aggregates must be delegated to the SVM; this action is only available using the ONTAP CLI in `diag` mode in ASA r2.
