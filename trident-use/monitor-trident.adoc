...
NOTE: By default, Trident's metrics are exposed on the target port `8001` at the `/metrics` endpoint. These metrics are *enabled by default* when Trident is installed. You can configure to consume Trident metrics over HTTPS on port `8444` as well. See <<Create a Prometheus ServiceMonitor>> for more information.
...
You should define a Prometheus target to gather the metrics and obtain information about the backends Trident manages, the volumes it creates, and so on. See link:https://prometheus-operator.dev/docs/developer/getting-started/[Using ServiceMonitors^].
