...
NOTE: Trident supports provisioning of storage on NetApp ASA r2 systems using the iSCSI and NVMe over TCP protocols. Because ASA r2 systems deliver a disaggregated ONTAP storage experience, refer to the following Knowledge Base article while provisioning volumes: link:https://kb.netapp.com/on-prem/ASAr2/ASAr2_KBs/su_create_by_SVM_admin_using_CLI_fails_with_error_No_candidate_aggregates_are_available_for_storage_services[Storage Unit create by SVM admin using CLI fails with error "No candidate aggregates are available for storage services"^].
*<<This NOTE will be updated after the 25.02 doc is finalized.>>*
