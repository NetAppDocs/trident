...
a|Enables or disables access to the `.snapshot` directory.

Valid values: `true`, `false`.
Default behavior might vary by ONTAP and Trident version. Set explicitly to ensure consistent behavior.
...
|`skipRecoveryQueue`
a|During volume deletion, bypass the recovery queue in storage and delete the volume immediately.
a|``
...
For information about FlexGroups and workloads that are appropriate for FlexGroups refer to the https://www.netapp.com/pdf.html?item=/media/12385-tr4571.pdf[NetApp FlexGroup volume Best Practices and Implementation Guide^].
