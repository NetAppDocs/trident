...
Astra Trident supports volumes with the default CVS service type on https://cloud.google.com/architecture/partners/netapp-cloud-volumes/service-types[GCP^]. For backends created with `storageClass=software`, volumes have a minimum provisioning size of 100 GiB. *NetApp recommends customers consume sub-1TiB volumes for non-production workloads*. CVS currently provides this feature under Controlled Availability and does not provide technical support.
...
* PVCs that are smaller than 100 GiB will result in Trident creating a 100 GiB CVS volume.
* PVCs that are between 100 GiB to 600 GiB will result in Trident creating a CVS volume of the requested size.
...
NOTE: NetApp Cloud Volumes Service for GCP does not support CVS-Performance or CVS volumes less than 100 GiB in size. To make it easier to deploy applications, Trident automatically creates volumes of the minimum size if a too-small volume is requested.
