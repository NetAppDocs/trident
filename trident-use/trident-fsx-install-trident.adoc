...
The following example enables the `ontapConfigurator` and specifies `authType`, `fsxnID`, `protocols` to create trident backends and storage classes: 
----
...
----
Enable the `ontapConfigurator` to and specify the `authType`, `fsxnID`, `protocols` to create trident backends and storage classes: 
----
{
   "clusterName": "<eks-cluster>",
   "addonName": "netapp_trident-operator",
