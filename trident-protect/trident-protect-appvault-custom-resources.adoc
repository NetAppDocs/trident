...
When you create an AppVault CR for Amazon S3, you need to ensure that the user credentials you provide have the necessary permissions to access the S3 bucket. The following is an example of a policy that grants the required permissions. You can apply this policy to the user that manages Amazon S3 CRs.
...
For more information, refer to to the examples in the https://docs.aws.amazon.com/AmazonS3/latest/userguide/example-policies-s3.html[Amazon S3 documentation^].
