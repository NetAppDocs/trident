...
* *iSCSI self-healing interval*: Determines the frequency at which iSCSI Self-healing runs (default: 5 minutes). You configure it to run more frequently by setting a smaller number or less frequently by setting a larger number.
...
----
helm install trident trident-operator-100.2402.0.tgz --set iscsiSelfHealingInterval=3m0s --set iscsiSelfHealingWaitTime=6m0s
----
--
.tridentctl
--
