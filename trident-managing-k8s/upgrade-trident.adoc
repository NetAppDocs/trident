...
* You can perform an upgrade from a release that is five or more releases behind the target release. However, we recommend performing a multi-step upgrade. The following releases require an interim upgrade step: 

. Releases earlier than ??? to ??? and above
. Releases earlier than ??? to ??? and above
