...
You can manually run an execution hook for an app if you want to test it, or if you want to re-run the hook manually after a failure. You need to have Owner, Admin, or Member permissions to create execution hooks. To manually run an execution hook, you need to create a resource backup, then you need to request that an execution hook be run against the backup.


.Step 1: Create a pods-only resource backup
[%collapsible%closed]
=====
