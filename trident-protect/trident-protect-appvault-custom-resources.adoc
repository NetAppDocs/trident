...
The `providerType` and `providerConfig` keys in an AppVault CR require specific values. Refer to the following table for supported values for the `providerType` key, and the associated `providerConfig` key that you need to use with each `providerType` value:
