...
** *`--no-rename`* - If you use the `--no-rename` flag, Trident retains the existing volume name while importing volumes, and manages the lifecycle of the volumes. This option is supported only for the `ontap-nas`, `ontap-san` (including ASA r2 systems), `ontap-san-economy`, and `ontap-nas-economy` (qtree import, Tech Preview) drivers.
+
...
[NOTE]
====
* Trident does not support FlexVol volume import using the `ontap-nas-economy` driver. To import qtrees with the `ontap-nas-economy` driver, refer to <<import-qtrees,Import qtrees>> (Tech Preview).
...
[NOTE]
====
Qtree import is a Tech Preview feature in Trident 26.06. Tech Preview features are not supported for production use. The behavior and annotations can change before general availability (GA).
====
...
[WARNING]
====
Do not point two Kubernetes clusters at the same active SVM, and do not import a qtree that another cluster still manages. If both clusters believe they manage the same volume, deleting it from one cluster removes it from the other.
====
