...
[.lead]
After you create Autogrow Policies, you can view, update, and delete them. You can also monitor which volumes use a given policy.
...
IMPORTANT: Changes affect all volumes that use the policy. When possible, test changes in a non-production environment first.
...
* *Immediate effect:* All volumes that use the policy adopt the new parameters at the next growth evaluation.
* *No volume restart required:* Changes apply to the next growth operation.
...
Autogrow Policies use finalizer protection to prevent accidental deletion while volumes actively use them.
...
. If volumes still use the policy, the deletion enters a `Deleting` state. Check which volumes are affected:
+
[source,console]
----
tridentctl get autogrowpolicy production-db-policy -o yaml
...
Autogrow supports ONTAP backends. Autogrow also supports Google Cloud NetApp Volumes (GCNV) for both NAS and SAN volumes.

== Known limitations

* *ONTAP NVMe raw block volumes:* Volumes created with ONTAP versions earlier than 9.16.1 do not support autogrow.
* *Existing volumes (brownfield deployments):* Autogrow might not work for existing volumes even if a valid Autogrow Policy is applied. This limitation is due to an ongoing migration of volume publications. To confirm that migration is complete, check the Trident controller logs for `"Migration completed"` messages.
...
The PVC annotation always takes precedence. If a PVC has the `trident.netapp.io/autogrowPolicy` annotation, Trident uses that value regardless of what the StorageClass specifies. Refer to link:automatic-volume-expansion.html#policy-precedence[Policy precedence] for details.
...
Set the PVC annotation to `"none"`. This setting overrides any StorageClass-level policy for that volume:
...
Trident stops expanding the volume. It creates no further resize requests for that volume, even if usage continues to increase beyond the `usedThreshold`.

For example, with this policy, Trident stops growing the volume after it reaches 100 GiB:
...
Yes. When you update a policy, all volumes that use that policy adopt the new parameters at the next growth evaluation. No volume restarts are required.
...
Policies use finalizer protection. If volumes still use the policy, deletion enters a `Deleting` state and waits until you remove all volumes from the policy.
...
After you remove all volumes, Trident releases the finalizer and deletes the policy.

=== Which backends does autogrow support?

Autogrow supports ONTAP backends. It also supports Google Cloud NetApp Volumes (GCNV) for both NAS and SAN volumes.

Autogrow supports the NFS, iSCSI, FCP, and NVMe protocols. NVMe raw block volumes require ONTAP 9.16.1 or later.

Existing volumes in brownfield deployments might require volume publication migration to complete before autogrow takes effect. To verify migration status, check the Trident controller logs:
