...
== Critical information about Astra Trident 23.07
...
== Using tridentctl locally
Typically, you will use `tridentctl` on a Kubernetes cluster running Astra Trident. However, you can use one of the following options to run `tridentctl` commands locally on any Kubernetes cluster:

.Export KUBECONFIG environment variable
[%collapsible%closed]
====
. Set the KUBECONFIG environment variable to the location of the kubeconfig file:
+
----
export KUBECONFIG=<path of that kubeconfig file>
----
. Use `tridentctl` to run commands as normal.
+
----
...
.Specify KUBECONFIG path in command
[%collapsible%closed]
====
You can specify the KUBECONFIG path for individual commands using `-k` or `--kubeconfig` as follows:
----
tridentctl -k /path/to/kubeconfigFile <cmd> -n <namespace>
----
====
