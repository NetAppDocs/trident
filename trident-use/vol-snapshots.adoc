...
summary: You can create snapshots of PVs at the Kubernetes layer and to maintain point-in-time copies of volumes created by Astra Trident and schedule creation of additional volumes (clones). Volume snapshot is supported by `ontap-nas`, `ontap-san`, `ontap-san-economy`, `solidfire-san`, `gcp-cvs`, and `azure-netapp-files` drivers.
---
...
You can create snapshots of PVs at the Kubernetes layer to maintain point-in-time copies of volumes created by Astra Trident and schedule creation of additional volumes (clones). Volume snapshot is supported by `ontap-nas`, `ontap-san`, `ontap-san-economy`, `solidfire-san`, `gcp-cvs`, and `azure-netapp-files` drivers.
