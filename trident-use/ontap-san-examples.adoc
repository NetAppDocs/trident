...
NOTE: While using “vsadmin” credentials, the aggregates must be delegated to the SVM. This action is only available using the ONTAP CLI in `diag` mode.
...
*If specified, set to `true` for ASA*.
