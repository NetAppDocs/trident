...
* Ensure that you provide valid AD users while enabling secure SMB.
* Secure SMB is not supported for unmanaged volume import with `ontap-nas` driver.

.Steps

. Specify smbAdminUser in TridentBackendConfig as shown in the following example:
+
[source,yaml]
----
...
parameters:
  ...
  smbUserPermission: full-control
----
