...
You can configure Trident protect AutoSupport connection settings such as a proxy and TLS verification. This affects network connections from Trident protect creates to upload support bundles to NetApp. You can configure a proxy with or without TLS verification for the upload connection.
...
--
.Steps
. Configure a proxy connection for Trident protect that uses TLS to secure the connection:
+
[source,console]
----
helm upgrade -n trident-protect trident-protect netapp/trident-protect --set autoSupport.proxy=http://myproxy.com
----
--
.Configure an insecure proxy connection
--
.Steps
. Configure a proxy connection for Trident protect that skips TLS verification for the connection:
+
[source,console]
----
helm upgrade -n trident-protect trident-protect netapp/trident-protect --set autoSupport.proxy=http://myproxy.com --set autoSupport.insecure=true
----
--
====