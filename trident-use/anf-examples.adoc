...
*The `ro` mount option is not supported for the Azure NetApp Files subvolume driver. If you specify `ro`, backend creation will fail.* 

*The `ro` mount option is strongly discouraged for all other Azure NetApp Files backends. If you specify `ro`, CSI operations will fail.*
