...
Astra Trident discovers all available iSCSI LIFs and uses them to establish the multipath session. 

A warning is generated if iSCSI LIFs are specified. |Derived by the SVM
...
|`username` |Username needed to commununicte with the ONTAP cluster. Used for credential-based authentication. |“”

|`password` |Password needed to commununicte with the ONTAP cluster. Used for credential-based authentication. |“”
