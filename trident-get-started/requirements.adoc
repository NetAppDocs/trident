...
* OpenShift 4.12, 4.14 - 4.22 (If you plan to use iSCSI node preparation with OpenShift 4.19, the minimum Trident version supported is 25.06.1.)
+
...
* On-premises FAS, AFF, or ASA r2 (iSCSI, NVMe/TCP, and FC) running ONTAP versions under NetApp full or limited support. See link:https://mysupport.netapp.com/site/info/version-support[Software Version Support].
...
== Trident support for Kubernetes on Kubernetes and OpenShift hosted control planes
 
Beginning with version 26.06, Trident fully qualifies the Kubernetes on Kubernetes topology. Kubernetes clusters whose nodes run as KubeVirt virtual machines on an infrastructure (host) cluster. This topology underpins hosted control plane offerings such as OpenShift hosted control planes (HCP) on OpenShift Virtualization.
 
NOTE: This configuration worked in earlier releases, but full qualification begins with 26.06.
 
The guest (tenant) cluster uses the KubeVirt CSI driver (`kubevirt-csi`) as its storage provider. When a workload in the guest cluster requests a `PersistentVolumeClaim`, the KubeVirt CSI driver creates a corresponding volume in the infrastructure cluster. Trident provisions the volume from ONTAP, and the volume is hot-plugged into the guest node VM. Trident runs only in the infrastructure cluster. The guest cluster consumes ONTAP storage indirectly through the KubeVirt CSI driver.
 
.Requirements:
* Trident is installed on the infrastructure cluster with a supported ONTAP backend.
* A Trident storage class exists in the infrastructure cluster for the KubeVirt CSI driver to map guest-cluster volumes onto.
* KubeVirt or OpenShift Virtualization is deployed on the infrastructure cluster, and the KubeVirt CSI driver is installed in each guest cluster.
...
|Kubernetes (infrastructure and guest cluster)
|1.36
...
.Points to consider:
* In the guest cluster, the volumes appear as hot-plugged block devices on the guest node VMs.
* The KubeVirt CSI driver workflow does not require Trident in the guest cluster. You configure the ONTAP backend and storage class in the infrastructure cluster.
 
.Install Trident directly in the guest cluster:
You can also install Trident directly in the guest (tenant) cluster. Install Trident in the guest cluster when you need the full Trident feature set, such as snapshots, clones, volume resize, and import, which `kubevirt-csi` does not provide on its own.
 
In the guest cluster, use IP-based protocols: NFS, iSCSI, and NVMe/TCP. The guest cluster does not support Fibre Channel (FC) because the guest nodes are virtual machines without direct FC connectivity.
...
=== Container images required for Trident 26.06
...
a|
* docker.io/netapp/trident:26.06.0                      
* docker.io/netapp/trident-autosupport:26.06                   
...
* docker.io/netapp/trident-operator:26.06.0 (optional)  
|
|===
