...
NOTE: When the aggregate list is updated in ONTAP, the list is updated in Trident automatically by polling ONTAP without having to restart the Trident Controller. When you have configured a specific aggregate in Trident to provision volumes, if the aggregate is renamed or moved in ONTAP, the backend will move to failed state in Trident while polling the ONTAP aggregate.
...
|flexgroupAggregateList | Aggregate for provisioning (optional; if set, must be assigned to the SVM). All aggregates assigned to the SVM are used to provision a FlexGroup Volume. Supported for *ontap-nas-flexgroup* storage driver. |
