...
Each backend provisions volumes in a single namespace on the Kubernetes cluster that hosts ADS. To create volumes in other namespaces, you can define additional backends.  Note that ADS volumes can be attached to any namespace in the hosting cluster, any other Kubernetes cluster, or anywhere else that may mount NFS shares.

You can control how each volume is provisioned by default by using these options in a special section of the configuration file.
