...
[.lead]
You can deploy Astra Trident using the Trident operator. 
...
== Trident operator deployment options
You can deploy the Trident operator in one of two ways: 
...
* Manually: Trident provides a file that can be used to install the operator and create associated objects. 
...
* You have full privileges to a supported Kubernetes cluster running a supported version of Kubernetes. Review the link:requirements.html[requirements].
...
* A link:requirements.html[supported Kubernetes version]
...
. Download and extract the latest version of the Trident installer bundle from link:https://github.com/NetApp/trident/releases/latest[the _Assets_ section on GitHub^].
+
----
