...
|`cloudProvider` | Set to `"Azure"` when using managed identities on an AKS cluster. |`""` (Check this ???)

|`cloudIdentity` |Set to workload identity when using cloud identity on an AKS cluster. |`""` (Check this ???)
