...
helm repo update
----
. Upgrade the Trident protect CRDs (required only if you are upgrading from a version earlier than 25.06.0)::
+
[source,console]
----
...
----
+
NOTE: This step is not required for version 25.06.0 and later, as the CRDs are now included in the Helm chart.
. Upgrade Trident protect:
+
[source,console]
----
