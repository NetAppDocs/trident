...
To import a LUKS volume, you must set `luksEncryption` to `true` on the backend. The `luksEncryption` option tells Astra Trident if the volume is LUKS (`true`) or not LUKS (`false`) as shown in the following example. 
