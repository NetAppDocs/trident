...
You can provision SMB volumes using the `ontap-nas` and `ontap-nas-economy` drivers. Before you complete <<ONTAP SAN and NAS driver integration>> complete the following steps. 
...
|`securityStyle` 
|Security style for new volumes. *Must be set to "ntfs" for SMB volumes.*
|"ntfs"
