...
[.lead]
Learn how to create and use ONTAP NAS drivers with your Astra Trident installation. This section provides backend configuration examples and details about how to map backends to StorageClasses.
...
The mount options for Kubernetes-persistent volumes are normally specified in a storage class using `mountOptions`, but if no mount options are specified in a storage class, Astra Trident will use mount options specified in the storage backend configuration file. 

If no mount options are specified in the storage class or the backend configuration file, Astra Trident will not set any mount options on an associated persistent volume.	

There are driver-specific options for `nfsMountOptions`. Refer to the documentation for your driver type for detailed settings.
...
.Minimal configuration for `ontap-nas-economy`
[%collapsible%open]
====
----
---
...
.Minimal configuration for `ontap-nas-flexgroup`
[%collapsible%open]
====
----
---
...
----
====
.Minimal configuration for SMB volumes
[%collapsible%open]
====
----
