...
* *Kubernetes*:
** Added support for CSI Volume Group Snapshots with `v1beta1` Volume Group Snapshot Kubernetes APIs for ONTAP-SAN iSCSI driver. See 
** Added support for ONTAP ASA r2 for NVMe/TCP in addition to iSCSI.
** Added secure SMB support for ONTAP-NAS and ONTAP-NAS-Economy volumes. Active Directory users and groups may now be used with SMB volumes for enhanced security.
...
** Added support for volume replication with SnapMirror for NVMe backends.
