...
For managed import(if lun is named lunFoo):
trident import volume BackendForiSCSIEco toimport/lunFoo -f import1.yaml
Note: Post first LUN inside toimport is imported the flexvol will be renamed and in the subsequent import LUNs(LUNs inside same flexvol) import statement should use the newly renamed flexvol name.
For example, if the second lun say lunBar previously inside toimport
which would be changed say trident_lun_pool_xyz import statement will change to:
trident import volume BackendForiSCSIEco trident_lun_pool_xyz/lunBar -f import2.yaml
Import of san-economy LUN/s are done one LUN at a time. which can be automated through a script.
