...
##You can exclude specific namespace annotations and labels using `restoreSkipNamespaceAnnotations` and `restoreSkipNamespaceLabels`. This is useful if you want to exclude certain namespaces that are not relevant to your backup and restore operations.##

[source,console]
----
helm install trident-protect netapp-trident-protect/trident-protect \
  --set clusterName=<name_of_cluster> \
  --set restoreSkipNamespaceAnnotations="{annotation1,annotation2}" \
  --set restoreSkipNamespaceLabels="{label1,label2}" \
  --version 100.2506.0 --create-namespace --namespace trident-protect 
----
