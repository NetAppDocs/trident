...
Astra Trident provides the `solidfire-san` storage driver to communicate with the cluster. Supported access modes are: _ReadWriteOnce_ (RWO), _ReadOnlyMany_ (ROX), and _ReadWriteMany_ (RWX).
