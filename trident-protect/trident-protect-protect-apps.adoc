...
+
##If a failover happens, the system can use these snapshots for up to 7 days to restore data. This approach makes the recovery process faster and more efficient because only the changes made since the last snapshot will be transferred, not all the data.##
