...
You can restore a backup to a different cluster if the original cluster is destroyed.
...
* The destination cluster has access to the backups stored in the object storage bucket.
* The respective AppVault CR is defined in the destination cluster.

.Steps
. Check the availability of AppVault CR on the destination cluster using Trident protect CLI plugin:
+
[source,console]
-----
...
-----
+
NOTE: Ensure that the namespace intended for the application restore does not exist on the destination cluster.
. View the backup contents of available AppVault from the destination cluster:
+
[source,console]
-----
...
+---------------+-----------+--------+-------------------------+---------------------------+---------------------------------------------------------------------------------------------------------------------+
| production 1 | wordpress | backup | wordpress-bkup-1        | 2024-10-30 08:37:40 (UTC) | backup path 1      |
| production 1 | wordpress | backup | wordpress-bkup-2 | 2024-10-30 08:43:03 (UTC) | backup path 2|
+---------------+-----------+--------+-------------------------+---------------------------+---------------------------------------------------------------------------------------------------------------------+

----
. Restore the application to the destination cluster using AppVault name and archive path:
+
// begin tabbed block
[role="tabbed-block"]
====
...
+
* *metadata.name*: (_Required_) The name of this custom resource; choose a unique and sensible name for your environment.
* *spec.appVaultRef*: (_Required_) TThe name of the AppVault where the backup contents are stored.
* *spec.appArchivePath*: The path inside AppVault where the backup contents are stored. You can use the following command to find this path:
+
[source,console]
-----
