...
You can run the `update backend state` command to suspend or resume backend operations.
----
Usage:
  tridentctl update backend state [flag]
----
Flags:
`--userstate`: Set to `suspended` to suspend backend operations. Set to `normal` to resume backend operations.
