...
|`userState` |Suspend or restore backend storage operations. 

When suspended, new volumes cannot be provisioned on the backend. Existing volumes remain available. 

`suspended` to suspend backend operations.

`normal` to resume backend operations. |No default; optional
