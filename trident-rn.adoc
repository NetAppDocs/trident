...
* Added OpenShift Service CA Operator integration to automate TLS certificate management for the Trident Protect admission webhook. Set `useOpenshiftServiceCA` to `true` in your Helm values to enable this feature. To learn more, refer to link:trident-protect/trident-protect-customize-installation.html[Customize Trident Protect installation].
