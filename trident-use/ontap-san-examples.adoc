...
In this minimum configuration example `clientCertificate`, `clientPrivateKey`, and `trustedCACertificate` (optional, if using trusted CA) are populated in `backend.json` and take the base64-encoded values of the client certificate, private key, and trusted CA certificate, respectively.
...
This configuration creates an `ontap-san` backend with `useCHAP` set to `true`.
...
This configuration creates an `ontap-san-economy` backend with `useCHAP` set to `true`.
...
* The `protection-gold` StorageClass will map to the first and  second virtual pool in the `ontap-nas-flexgroup` backend and the first virtual pool in the `ontap-san` backend. These are the only pool offering gold level protection.
+
----
...
* The `protection-not-gold` StorageClass will map to the third and fourth virtual pool in `ontap-nas-flexgroup` backend and the second and third virtual pool in `ontap-san` backend. These are the only pools offering protection level other than gold.
+
----
apiVersion: storage.k8s.io/v1
kind: StorageClass
...
* The `app-mysqldb` StorageClass will map to the fourth virtual pool in `ontap-nas` backend and the third virtual pool in `ontap-san-economy` backend. These are the only pools offering storage pool configuration for mysqldb type app.
+
----
apiVersion: storage.k8s.io/v1
kind: StorageClass
...
* The `protection-silver-creditpoints-20k` StorageClass will map to the third virtual pool in `ontap-nas-flexgroup` backend and the second virtual pool in `ontap-san` backend. These are the only pools offering gold-level protection at 20000 creditpoints.
+
----
apiVersion: storage.k8s.io/v1
kind: StorageClass
...
parameters:
  ...
  fsType: "ext4"
----
* The `creditpoints-5k` StorageClass will map to the second virtual pool in `ontap-nas-economy` backend and the third virtual pool in `ontap-san` backend. These are the only pool offerings at 5000 creditpoints.
+
----
apiVersion: storage.k8s.io/v1
kind: StorageClass
