...
In `/deploy/operator.yaml`, update `image: docker.io/netapp/trident-operator:23.07.1` to reflect the location of your image registry. Your link:../trident-get-started/requirements.html#container-images-and-corresponding-kubernetes-versions[Trident and CSI images] can be located in one registry or different registries, but all CSI images must be located in the same registry. For example:

* `image: <your-registry>/trident-operator:23.07.1` if your images are all located in one registry.
* `image: <your-registry>/netapp/trident-operator:23.07.1` if your Trident image is located in a different registry from your CSI images. 
