...
* To specify cloud identity, get the `client ID` from your user-assigned managed identity and specify in 'azure.workload.identity/client-id: xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxx'. To create user-assigned managed identity, refer to link:https://learn.microsoft.com/en-us/entra/identity/managed-identities-azure-resources/how-manage-user-assigned-managed-identities?pivots=identity-mi-methods-azp[Microsoft: Manage user-assigned managed identities].
