...
NOTE: By default, Trident protect collects support information from clusters that helps with any NetApp support cases that you might open, including logs, metrics, and topology information about the clusters and managed applications. These support bundles are sent to NetApp Support on a daily schedule. You can optionally disable this support bundle collection when you install Trident protect. You can manually link:trident-protect-generate-support-bundle.html[generate a support bundle] at any time.
