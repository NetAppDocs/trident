...
* Added support for different Azure clouds, such as Azure Government and Azure China, and custom cloud configuration for Azure NetApp Files backends.
...
* Added support for auto-tiering for Google Cloud NetApp Volumes. You can now optimize storage costs by now Add Kubernetes-native controls to configure ONTAP FabricPool auto-tiering for Google Cloud NetApp Volumes (GCNV) through NetApp Trident.
...
* Kubernetes: Fixed an issue where unpublishing a read-only clone removed export policy rules from the source volume in ONTAP-NAS, ONTAP-NAS-Economy, and Google Cloud NetApp Volumes drivers link:https://github.com/NetApp/trident/issues/1086[Issue #1086].
* Kubernetes: Switched kubectl images to lightweight Alpine-based variants to prevent pull failures following Bitnami's public image deprecation https://github.com/NetApp/trident/issues/1080[Issue #1080].
* Kubernetes: Fixed preserving annotation of existing deployment during Trident upgrade link:https://github.com/NetApp/trident/issues/1004[Issue #1004].
* Kubernetes: Allow clone across different storage classes if both storage classes are pointing to the same backend link:https://github.com/NetApp/trident/issues/1104[Issue #1104].
...
* Kubernetes: Improved CSI and ONTAP clone split timeouts to avoid race conditions with some backup applications link:https://github.com/NetApp/trident/issues/1098[Issue #1098], link:https://github.com/NetApp/trident/issues/1100[ #1100].
* Kubernetes: Fixed suppression of LUKS error messages link:https://github.com/NetApp/trident/issues/1069[Issue #1069].
