...
Beginning with the 25.10 release, NetApp Trident supports link:https://docs.netapp.com/us-en/ontap-afx/index.html[NetApp AFX storage system^]. NetApp AFX storage systems differ from other ONTAP systems (ASA, AFF, and FAS) in the implementation of their storage layer.

NOTE: Only the `ontap-nas` driver (with NFS protocol) is supported for  AFX systems; SMB protocol is not supported.
