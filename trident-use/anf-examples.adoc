...
|`networkFeatures` footnote:1[] |Set of VNet features for a volume, may be `Basic` or `Standard` |""

|`nfsMountOptions` footnote:2[] |Fine-grained control of NFS mount options. |"nfsvers=3"
...
| `nasType` | `nfs` or `smb` | `nfs` (null configures NFS)
...
--
Footnotes:
footnote:1[]: https://docs.microsoft.com/en-us/azure/azure-netapp-files/configure-network-features[Network Features^] is not available in all regions and might have to be enabled in a subscription.  Specifying `networkFeatures` when the functionality is not enabled will cause Astra Trident volume provisioning to fail.
footnote:2[]: To mount volumes using NFS version 4.1, you can include ``nfsvers=4`` in the comma-delimited mount options list to choose NFS v4.1. Mount options set in a storage class definition override mount options set in backend configuration.
--
...
|===
|Parameter |Description |Default
|`exportRule` footnote:1[] |The export rule(s) for new volumes |"0.0.0.0/0"
...
--
Footnote:
footnote:1[]: The `exportRule` value must be a comma-separated list of any combination of IPv4 addresses or IPv4 subnets in CIDR notation.
--
...
=== Example definition for SMB volumes
...
[NOTE]
====
* `nasType: "smb"` filters for pools which support SMB volumes. `nfs` or null filters NFS pools.

* `smbcreds` is the Secret name containing your Active Directory username and password.
====
