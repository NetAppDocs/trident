...
The `gcp-cvs` driver uses virtual storage pools to abstract the backend and allow Astra Trident to determine volume placement. The administrator defines the virtual storage pools in the `backend.json` files. Storage classes use labels to identify the virtual storage pools.

* If virtual pools are defined in the backend, Astra Trident will try to create a volume in one of those pools only.
* If virtual pools are not defined in the backend, Astra Trident will select a pool from the available pools in the region.
...
NOTE: Replication is not managed by Astra Trident.
