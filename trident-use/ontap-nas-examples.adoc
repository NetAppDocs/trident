...
|`dataLIF` |IP address of protocol LIF. Can be changed after initial setting. Refer to <<Update `dataLIF` after initial configuration>>. 

A fully-qualified domain name (FQDN) can be specified. The FQDN will be used for the NFS mount operations to allow you to create a round-robin DNS to load-balance across multiple data LIFs.
...
If `dataLIF` is not provided, Astra Trident will fetch the IPv6 data LIFs from the SVM. 
...
Cannot be modified after creation. To update this parameter, you will need to create a new backend.|“trident”
...
=== Backends with virtual storage pools
...
[NOTE] 
====
* If PVCs are attached to any pod, Astra Trident will bounce the pods to use the new data LIF.
* If a PVC is attached to multiple pods, Astra Trident will bring down all pods and bring them back up using the new data LIF.
====
