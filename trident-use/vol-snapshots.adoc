...
WARNING: The PVC will be created in the same backend as the source volume for `ontap-san`, `ontap-san-economy`, `ontap-nas`, and `ontap-nas-flexgroup` drivers. Refer to link:https://kb.netapp.com/Cloud/Astra/Trident/Creating_a_PVC_from_a_Trident_PVC_Snapshot_cannot_be_created_in_an_alternate_backend[KB: Creating a PVC from a Trident PVC Snapshot cannot be created in an alternate backend^].
