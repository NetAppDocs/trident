...
| Active Directory admin user or user group with full access to SMB shares. Use this parameter to enable secure SMB and restrict access to SMB shares using ACLs. |
...
----
NOTE: Ensute that you add `annotations` to enable secure SMB. Secure SMB does not work without the annotations, irrespective of configurations set in the Backend or PVC.
