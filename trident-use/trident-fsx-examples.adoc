...
|`igroupName` 
|Astra Trident creates and manages per-node `igroups` as `<nodeName>-<tridentUUID>` for `ontap-san` and `ontap-san-economy` drivers. Per-backend `igroups` have been deprecated.

Due to ONTAP `igroup` naming constraints, Astra Trident may share `igroups` between multiple cluster hosts if cluster host names are too long. Refer to link:https://docs.netapp.com/us-en/ontap-cli-96/lun-igroup-create.html[lun igroup create^] for details about `igroup` naming.

|`<nodeName>-<tridentUUID>`
