...
The steps in this section cover how to create the backend separately. For information on how to use the Automatic Backend feature, refer to the link:trident-fsx-storageclass-pvc.html#configure-a-kubernetes-storageclass-object-with-automatic-backend-configuration-support-for-fsx-for-ontap[Configure StorageClass and PVC] section for more information.
...
Before you complete link:trident-fsx-storage-backend.html#ontap-san-and-nas-driver-integration[ONTAP SAN and NAS driver integration] complete these steps: link:worker-node-prep.html#prepare-to-provision-smb-volumes.html[Prepare to provision SMB volumes].
