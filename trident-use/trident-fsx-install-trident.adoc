...
The following example enables the `ontapConfigurator` and specifies `authType`, `fsxnID`, `protocols` to create Trident backend and storage classes soon after installation: 
----
helm install trident trident-operator-100.2410.0.tgz --set cloudProvider=$CP --set cloudIdentity=$CI -–namespace trident --set ontapConfigurator.enabled=true  --set ontapConfigurator.svms[0].fsxnID="fs-0dfeaa884a68b1cab"  --set ontapConfigurator.svms[0].protocols[0]=iscsi  --set ontapConfigurator.svms[0].protocols[1]=nfs  --set ontapConfigurator.svms[0].authType="awsarn"
----
+
...
----
Enable the `ontapConfigurator` to and specify the `authType`, `fsxnID`, `protocols` to create Trident backend and storage classes automatically soon after the Trident installation: 
----
{
   "clusterName": "<eks-cluster>",
   "addonName": "netapp_trident-operator",
