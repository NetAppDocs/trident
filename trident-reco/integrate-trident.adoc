...
=== Cloud Volumes Service with GCP backend drivers
...
Astra Trident can create CVS volumes in one of two link:https://cloud.google.com/architecture/partners/netapp-cloud-volumes/service-types[service types^]:

* *CVS*: The base CVS service type providing high zonal availability with limited to moderate performance levels. The CVS service type uses storage pools to support volumes as small as 1 GiB without performance impact. A storage pool can serve as a container for up to 50 volumes where all volumes share the capacity and performance of the pool.
* *CVS-Performance*: Performance-optimized service type best suited for production workloads that value performance. The CVS-Performance service type can support small volumes with possible performance impacts. You can choose from link:https://cloud.google.com/architecture/partners/netapp-cloud-volumes/service-levels#service_levels_for_the_cvs-performance_service_type[three unique service levels^]: `standard`, `premium`, and `extreme`.
...
|Expand
|Replication
| `gcp-cvs` | Yes | Yes | Yes | Yes | Yes | Yes; Not managed by Astra Trident |
|===

The `gcp-cvs` driver uses virtual storage pools. Virtual storage pools abstract the backend, letting Astra Trident decide volume placement. The administrator defines the virtual storage pools in the backend.json files. Storage classes identify the virtual storage pools with the use of labels.
