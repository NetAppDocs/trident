...
. Use `helm install` and specify a name for your deployment as in the following example where `24.02.0` is the version of Astra Trident you are installing. 
+
----
...
For example, to change the default value of `debug`, run the following `--set` command where `24.02.0` is the version of Astra Trident you are installing:
...
|`cloudIdentity` |Set to workload identity when using cloud identity on an AKS cluster. |`""`
