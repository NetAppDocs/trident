...
* ONTAP NAS: If dataLIF is not provided, Astra Trident will fetch the IPv6 data LIFs from the SVM. link:trident-use/ontap-nas-examples.html[ONTAP NAS configuration options and examples] for details
...
You can update the Data LIF on and ONTAP NAS backend only. You cannot specify or update `dataLIF` on an ONTAP SAN backend. 
