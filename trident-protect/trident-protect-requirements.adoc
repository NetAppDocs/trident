...
* Ensure that the cluster on which you install Trident Protect is configured with a running snapshot controller and the related CRDs. To install a snapshot controller, refer to https://docs.netapp.com/us-en/trident/trident-use/vol-snapshots.html#deploy-a-volume-snapshot-controller[these instructions].
* Ensure that at least one VolumeSnapshotClass exists. For more information, refer to link:https://docs.netapp.com/us-en/trident/trident-reference/objects.html#kubernetes-volumesnapshotclass-objects[VolumeSnapshotClass].
* Helm 4.x or later is required to install Trident Protect.
====
...
##To learn how to define applications and restore VMs with Trident Protect, see link:trident-protect-kubevirt-vms.html[Protect KubeVirt virtual machines with Trident Protect].##
...
NOTE: Many enterprise environments implement strict firewall policies across WAN connections. Verify these network requirements with your infrastructure team before configuring replication.
