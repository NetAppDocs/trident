...
In `/deploy/operator.yaml`, update `image: docker.io/netapp/trident-operator:23.01.0` to reflect the location of your image registry. For example:.

* `image: <your-registry>/trident-operator:23.01.0` if your images are all located in one registry.
* `image: <your-registry>/netapp/trident-operator:23.01.0` if your Trident image is located in a different registry from your CSI images. 
