...
== Overview
Group volume snapshot is supported by `ontap-nas`, `ontap-nas-flexgroup`, `ontap-san`, `ontap-san-economy`, `solidfire-san`, `gcp-cvs`, `azure-netapp-files`, and `google-cloud-netapp-volumes` drivers.
...
You'll also need to modify the "external-snapshotter" YAML in Trident to ensure the feature is enabled:
...
A dynamic VolumeGroupSnapshot is a group snapshot taken by a CSI provisioner.

Here are the steps (at a high-level) for creating a VolumeGroupSnapshot.

Create a VolumeGroupSnapshotClass at any time prior to creating a VolumeGroupSnapshot. 
Create a PVC with a particular label KVP example: "consistentGroupSnapshot: groupA" (assumes you already setup a storage class).
Create a VolumeGroupSnapshot with the same label specified.
