...
Supported only for manual QoS capacity pools. |`4 MiB/sec`
|===
...
== Consider Azure cloud environments (26.06)
Starting with the 26.06 release, Trident supports creating and managing Azure NetApp Files backends in multiple Azure cloud environments.
...
NOTE: If the vNet is located in a different resource group from the Azure NetApp Files (ANF) storage account, specify the resource group for the virtual network while configuring the resourceGroups list for the backend.
...
|`snapshotDir` |Access to the `.snapshot` directory |`true`, `false` (Set explicitly).
...
[%collapsible%closed]
====
This backend configuration places volumes in Azure's `eastus` location in an `Ultra` capacity pool.
Trident automatically discovers all of the subnets delegated to Azure NetApp Files in that location and places a new volume on one of them randomly.
