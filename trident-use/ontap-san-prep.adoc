...
Beginning with the 25.02 release, Trident automatically detects ASA r2 personality of the ONTAP 9.16.1 system and enables the provision and usage of storage on ASA r2 via the iSCSI protocol.

== Requirements
For all ONTAP backends, Trident requires at least one aggregate assigned to the SVM.

NOTE: If using "adming" credentials, an aggregate need not be assigned to the SVM. While using “vsadmin” credentials with ASA r2, the aggregates must be delegated to the SVM; this action is only available using the ONTAP CLI in `diag` mode in ASA r2.
