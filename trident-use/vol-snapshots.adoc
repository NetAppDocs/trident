...
To import the volume snapshot, Astra Trident uses the PV name and `VolumeSnapshotContent` annotation to locate the backend snapshot and name the Trident `VolumeSnapshot` it creates. 
...
* The `annotations` must be `trident.netapp.io/internalSnapshotName: <backend-snapshot-name>`.
* The `snapshotHandle` must be `<pv-name>/<volumeSnapshotContentName>`. This is the only information provided to Astra Trident by the external snapshotter in the `ListSnapshots` call.
+
NOTE: The `<volumeSnapshotContentName>` cannot always match the backend snapshot name due to CR naming constraints. 
* `VolumeSnapshotRef` is the name of the Trident `VolumeSnapshot` that will be created.
+
.Example
The following example creates `VolumeSnapshotContent` that references backend snapshot `snap-01`.
+
----
apiVersion: snapshot.storage.k8s.io/v1
...
. *Cluster admin:* Bind the `VolumeSnapshot` to the `VolumeSnapshotContent`.
+
.Example
The following example binds `VolumeSnapshot` named `test-snapshot` to `VolumeSnapshotContent` named `my-trident-snapshot-content`.
+
----
apiVersion: snapshot.storage.k8s.io/v1
kind: VolumeSnapshot
...
+
.Example
The following example creates a PVC referencing the `VolumeSnapshot` named `test-snapshot`. 
+
----
apiVersion: v1
kind: PersistentVolumeClaim
