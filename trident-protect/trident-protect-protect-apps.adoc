...
[NOTE] 
====
* You can create schedules for snapshots only by setting `backupRetention` to 0 and `snapshotRetention` to a value greater than 0. Setting `snapshotRetention` to 0 means any scheduled backups will still create snapshots, but those are temporary and get deleted immediately after the backup is completed.
