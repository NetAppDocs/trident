...
|`autosupportImage` |The container image for Autosupport Telemetry |"netapp/trident-autosupport:23.04.0"
...
|`controllerPluginNodeSelector` |Additional node selectors for Trident Node Pods.	See link:trident-concepts/intro.html#astra-trident-architecture[Astra Trident architecture] for details. 

Follows same format as `pod.spec.nodeSelector`. |No default; optional

|`controllerPluginTolerations` |Overrides Kubernetes tolerations for Trident Controller Pods and transient Pods created by the operator. See link:trident-concepts/intro.html#astra-trident-architecture[Astra Trident architecture] for details.
