...
|===
|Parameter |Description |Driver type
|`version` | Always 1 |
...
Can be set to use IPv6 addresses if Astra Trident was installed using the `--use-ipv6` flag. IPv6 addresses must be defined in square brackets, such as [28e8:d9fb:a825:b7bf:69a8:d02f:9e7b:3555].  |NAS and SAN
...
Using the `autoExportPolicy` and `autoExportCIDRs` options, Astra Trident can manage export policies automatically. |NAS and SAN

|`autoExportCIDRs` |List of CIDRs to filter Kubernetes’ node IPs against when `autoExportPolicy` is enabled. 

Using the `autoExportPolicy` and `autoExportCIDRs` options, Astra Trident can manage export policies automatically.	|NAS and SAN

|`labels` |Set of arbitrary JSON-formatted labels to apply on volumes |NAS and SAN

|`clientCertificate`	|Base64-encoded value of client certificate. Used for certificate-based auth |NAS and SAN

|`clientPrivateKey`	|Base64-encoded value of client private key. Used for certificate-based auth	|NAS and SAN

|`trustedCACertificate` |Base64-encoded value of trusted CA certificate. Optional. Used for certificate-based auth |NAS and SAN

|`username` |Username to connect to the cluster or SVM. Used for credential-based authentication. For example, vsadmin.|NAS and SAN

|`password` |Password to connect to the cluster or SVM. Used for credential-based authentication. |NAS and SAN

|`svm` |Storage virtual machine to use |NAS and SAN

|`igroupName` |Name of the igroup for SAN volumes to use. Refer to <<Details about `igroupName`>>. |SAN

|`storagePrefix` |Prefix used when provisioning new volumes in the SVM. 

Cannot be modified after creation. To update this parameter, you will need to create a new backend.|NAS and SAN
...
|`lunsPerFlexvol` |Maximum LUNs per Flexvol, must be in range [50, 200] |NAS and SAN

|`debugTraceFlags` |Debug flags to use when troubleshooting. Example, {“api”:false, “method”:true} 

Do not use `debugTraceFlags` unless you are troubleshooting and require a detailed log dump.|NAS and SAN
...
If no mount options are specified in the storage class or the configuration file, Astra Trident will not set any mount options on an associated persistent volume.	|NAS

|`qtreesPerFlexvol`	|Maximum Qtrees per FlexVol, must be in range [50, 300]	|NAS
...
`useREST` is not supported with MetroCluster.|NAS and SAN
...
|`chapInitiatorSecret` |CHAP initiator secret. Required if `useCHAP=true` |NAS

|`chapTargetInitiatorSecret` |CHAP target initiator secret. Required if `useCHAP=true` |NAS

|`chapUsername` |Inbound username. Required if `useCHAP=true` |NAS

|`chapTargetUsername` |Target username. Required if `useCHAP=true` |NAS
