...
. Select image::../media/icon_arrow[Arrow icon] next to Users and Roles.
...
. On the storage system, create a new role using ONTAP System Manager or use the following command in the ONTAP CLI:.

NOTE: Use this file <<Link>> to get the the list of ReST APIs and ZAPI commands used by Trident.

. Create a user in ONTAP System Manager or use this command to create a user name:+

. Add the role to the user in ONTAP System Manager or use the following command:
