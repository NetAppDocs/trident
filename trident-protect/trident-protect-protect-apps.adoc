...
.Before you begin
Ensure that the AWS session token expiration is sufficient for any long-running backups. If the token expires during the backup operation, the backup operation can fail.
