...
*Examples for importing volumes using PVC annotations*:
...
.Managed volume
--
The following example imports 1Gi ontap-nas volume `ontap_volume1` on backend with uuid `81abcb27-ea63-49bb-b606-0a5315ac5f21` and RWO mode set using PVC annotations:
...
.Unmanaged volume
--
The following example imports 1Gi `ontap-nas` volume `<nas-volume2>` on backend with uuid `34abcb27-ea63-49bb-b606-0a5315ac5f34` and RWO mode setas an unmanaged volume using PVC annotations:
...
metadata:
  name: <PVC-name>
  annotations:
    trident.netapp.io/importOriginalName: "<nas-volume2>"
