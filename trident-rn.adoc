...
* Added an annotation to the schedule CR to configure PVC bind timeout, which will be used by the backup CR: `protect.trident.netapp.io/pvc-bind-timeout-sec`. See link:trident-protect-protect-apps.html[Supported backup and schedule annotations^].
