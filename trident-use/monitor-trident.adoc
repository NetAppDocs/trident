...
If you are using the `tridentctl install` command, you can pass the `--metrics-https` flag to enable HTTPS metrics. If you are using YAML files, you can add the `--metrics-https` flag to the `trident-main` container in the `trident-deployment.yaml` file.
