...
The steps in this section cover how to create the backend separately. For information on how to use the Automatic Backend feature, refer to the xref:trident-fsx-storageclass-pvc.html#configure-a-kubernetes-storageclass-object-with-automatic-backend-configuration-support-for-fsx-for-ontap[Configure StorageClass and PVC] section for more information.
