...
.Managed volume
--
The following example imports 1Gi ontap-nas volume `<ontap-volume-name>` on backend `<backend-uuid>` using PVC annotations:
...
metadata:
  name: <PVC-name>
  annotations:
    trident.netapp.io/importOriginalName: "<ontap-volume-name>"
...
.Unmanaged volume
--
The following example imports 1Gi `ontap-nas` volume `<ontap-volume-name>` on backend `<backend-uuid>` as an unmanaged volume using PVC annotations:
