...
*-E nodiscard:*: Do not attempt to discard blocks at mkfs time (discarding blocks initially is useful on solid state devices and sparse / thin-provisioned storage). Use `-E nodiscard` for ext3 and ext4 file systems, and `-K` for xfs file system.
