...
| `cloudConfiguration` a| Optional. Specifies a named or a custom cloud environment: 
  - Specify a named cloud environment using the `cloudName` parameter. Supported values: `AzurePublic` (default),  `AzureChina`, or `AzureGovernment`
  (OR) 
  - Specify a custom Azure cloud environment with the following parameters:
...
IMPORTANT: You can specify either a named cloud environment (`cloudName`) OR a custom Azure cloud environment ( using `adAuthorityHost`, `audience`, and `endpoint`), but NOT both.
  | `AzurePublic`
