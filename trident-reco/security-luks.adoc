...
----
+
NOTE: To import a LUKS volume, you must set `luksEncryption` to `true`. The `luksEncryption` option tells Astra Trident if the volume is LUKS (`true`) or not LUKS (`false`).  
