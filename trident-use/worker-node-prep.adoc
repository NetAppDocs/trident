 ...
 * If a mix of older, backend-scoped Trident igroups (Trident v22.10 or earlier) and current per-node igroups (Trident 23.04 or later) are detected to be in use, iSCSI self-healing scans for a single LUN at a time, instead of all LUNs. 

* If you are upgrading from a previous version of Trident that used backend-scoped igroups (like v22.10 and earlier), perform the following steps:
+
. Disable iSCSI self-healing and upgrade Trident to a newer version that uses per-node igroups.
. Unpublish each volume everywhere in the cluster and then republish them by deleting and recreating the workload consuming them. 
. Create a per-node igroup, add the host initiator IQN, and map that LUN to that igroup.
. If you are using RWX volumes, unpublish them everywhere in the cluster.
+
** Do this for each LUN that is mapped to the old Trident igroup; ensure that no LUNs are mapped to the old igroup, and all Kubernetes nodes which have LUNs attached have their own, isolated per-node igroup. 
. Re-enable iscsi-self healing.
