...
* ##ASTRACTL-36392, doc ticket-ASTRACTL-37052## Trident Protect now automatically disables protection schedules and cancels in-progress operations before an in-place restore, and re-enables them after the restore completes. To learn more, refer to link:trident-protect/trident-protect-restore-apps.html[Restore applications using Trident Protect^].
* ##ASTRACTL-36719, doc ticket-ASTRACTL-36882## Added the `runImmediately` field to the schedule CR and `--run-immediately` CLI flag to trigger an immediate backup or snapshot upon schedule creation. To learn more, refer to link:trident-protect/trident-protect-protect-apps.html#create-a-data-protection-schedule[Create a data protection schedule^].
* ##ASTRACTL-36849## Added support for specifying a custom name for the restored application using the `destinationApplicationName` field in the restore CR or the `--destination-app-name` CLI flag. To learn more, refer to link:trident-protect/trident-protect-restore-apps.html[Restore applications using Trident Protect^].
...
* Fixed incorrect PVC restoration in multi-namespace applications with identically named PVCs across namespaces, which could result in data loss. ##for this issue, a note will be added to down-level repos, see ASTRACTL-36941##
