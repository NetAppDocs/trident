...
Learn about how to create and use an Element backend with your Trident installation.
...
The `solidfire-san` storage driver supports both volume modes: file and block. For the `Filesystem` volumeMode, Trident creates a volume and creates a filesystem. The filesystem type is specified by the StorageClass.
