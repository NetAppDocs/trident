...
|`enableForceDetach` |`ontap-san`, `ontap-san-economy`, and `ontap-nas-economy` only. 
...
Force detach is available for `ontap-san`, `ontap-san-economy` and `onatp-nas-economy` only. Before enabling force detach, non-graceful node shutdown (NGNS) must be enabled on the Kubernetes cluster. For more information, refer to link:https://kubernetes.io/docs/concepts/architecture/nodes/#non-graceful-node-shutdown[Kubernetes: Non Graceful node shutdown^]. 

NOTE: For `ontap-nas-economy`, the `autoExportPolicy` parameter in the backend configuration should be set to `true` so that Trident can restrict access from the Kubernetes node with the taint applied using managed export policies.
