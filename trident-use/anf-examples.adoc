...
NOTE: Trident does not support Manual QoS capacity pools. In NetApp Trident 25.06, manual QoS capacity pools is supported as a tech preview.
...
| `qosType` | Represents the QoS type. 
*Tech preview for Trident 25.06*|  

| `maxThroughput` | Sets the maximum throughput allowed.
*Tech preview for Trident 25.06* |  `4MiB/sec`
...
[%collapsible%closed]
====
This backend configuration places volumes in Azure's `eastus` location with manual QoS.
