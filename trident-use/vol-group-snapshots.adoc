...
NOTE: VolumeGroupSnapshot is a beta feature and has beta APIs.
...
* If a clone is made using a grouped snapshot and then the group is to be deleted, a split-on-clone operation will begin and the group cannot be deleted until the split is complete.
