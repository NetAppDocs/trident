...
You can specify the following in the configuration file for ONTAP to work with AWS FSx and Secret Manager:

- `fsxFilesystemID`: Specify the ID of the AWS FSx file system. 
- `apiRegion`: AWS API region name.
- `apikey`: AWS API key.
- `secretKey`: AWS secret key.
Refer to link:https://aws.amazon.com/secrets-manager/[AWS Secrets Manager] for more information.|`""`
