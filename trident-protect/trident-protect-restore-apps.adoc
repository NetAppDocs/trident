...
* You can use advanced settings to customize restore operations. To learn more, refer to link:../trident-protect/trident-protect-restore-settings.html[Use advanced Trident Protect restore settings].
* To restore specific KubeVirt virtual machines (VMs) from an archive without restoring all namespace resources, use the `includedVirtualMachines` field on a restore CR. For supported CRs, field reference, and YAML examples, refer to link:trident-protect-kubevirt-vms.html[Protect KubeVirt virtual machines with Trident Protect].
====
...
* *spec.destinationApplicationName*: (_Optional_) The name for the restored application. If provided, the restored application uses this name. If not provided, the restored application uses the source application name.
...
.Steps
. Verify that the AppVault CR exists on the destination cluster using the Trident Protect CLI plugin:
+
[source,console]
----
...
----
+
NOTE: If the AppVault CR does not exist on the destination cluster, create it by following the steps in link:trident-protect-appvault-custom-resources.html[Use Trident Protect AppVault objects to manage buckets].
. View the backup contents of the available AppVault on the destination cluster, and note `appArchivePath` of the backup you want to restore:
+
[source,console]
----
...
NOTE: When using a CR, ensure that the namespace intended for the application restore exists on the destination cluster.
...
* *metadata.name*: (_Required_) The name of this custom resource; choose a unique and sensible name for your environment.
* *spec.appVaultRef*: (_Required_) The name of the AppVault where the backup contents are stored.
* *spec.appArchivePath*: (_Required_) The path inside AppVault where the backup contents are stored. Use the command from step 2 to view the backup contents and find `appArchivePath` for the backup you want to restore.
+
* *spec.destinationApplicationName*: (_Optional_) The name for the restored application. If provided, the restored application uses this name. If not provided, the restored application uses the source application name.
* *spec.namespaceMapping*: The mapping of the source namespace of the restore operation to the destination namespace. Replace `my-source-namespace` and `my-destination-namespace` with information from your environment.
+
