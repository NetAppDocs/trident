...
+
* ##ASTRACTL-36609 update-*Full backup scheduling*: Use the `--full-backup-rule` flag to schedule non-incremental full backups. This flag only works with `--granularity Daily`. Possible values:##
+
** ##`Always`: Create a full backup every day.##
** ##Specific weekdays: Specify one or more days separated by commas (for example, `"Monday,Thursday"`). Valid values: Monday, Tuesday, Wednesday, Thursday, Friday.##
+
