...
** Added support for force detach for ONTAP-NAS volumes during Non-Graceful Node Shutdown scenarios. All new ONTAP-NAS volumes will now use per-volume export policies while Trident manages them. Provided an upgrade path for existing volumes to be switched to the new export policy model on unpublish without impacting active workloads.
...
* Added host WWPN to igroup for Fibre Channel.
* Added iSCSI connection state discovery and logging when iSCSI sessions should be logged in but are not (Issue #961).
* Added support for GCNV SMB volumes.
* Added support for ONTAP volumes to skip recovery queue on deletion.
* Added support for ONTAP ASA R2 driver for iSCSI.
* Added parallelism for ONTAP-NAS driver for Trident node.
* Added automatic backend configuration for AWS FSxN.
...
** Added capability to control freeze behavior at application level for Kubevirt environments
** Added support for configuring AutoSupport proxy connections
** Added ability to define a secret for data mover encryption (Kopia / Restic)
** Added ability to manually run an execution hook
** Added ability to configure Security Context Constraints (SCCs) during Trident protect installation
** Added support for configuring node selector and affinity rules during Trident protect installation
** Added support for HTTP / HTTPS egress proxy for AppVault objects
** Added SESSION_TOKEN support to AWS S3 AppVault credentials
** Extended ResourceFilter to allow exclusion of Cluster Scoped Resources
** Added support for AWS Session Token in S3 AppVault credentials
...
** Fixed TridentBackendConfig credentials to support AWS ARN partitions (link:https://github.com/NetApp/trident/issues/913[Issue #913]).
...
== Trident data protection and data management features
* Added Backup and Restore support for KubeVirt / OpenShift Virtualization VMs for both volumeMode: File and volumeMode: Block (raw device) storage, to already available storage replication through AppMirrorReplication.
...
* Added tech preview for Fibre Channel support on ONTAP-SAN driver. Refer to link:./trident-use/fcp.html[Fibre Channel support].
...
* Added FSx personality to ONTAP drivers to enable integration with AWS IAM and SecretsManager, and to enable Trident to delete FSx volumes with backups (link:https://github.com/NetApp/trident/issues/453[Issue #453]).
