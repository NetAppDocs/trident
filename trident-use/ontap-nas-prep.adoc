...
* Certificate-based: This mode required a certificate installed on the backend for Astra Trident to communicate with an ONTAP cluster using the certificate. Here, the backend definition must contain Base64-encoded values of the client certificate, key, and the trusted CA certificate if used (recommended).
