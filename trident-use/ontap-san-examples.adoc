...
*Do not specify for ASA r2*.|"" (not enforced by default)
...
*If specified, always set to `true` for ASA r2*. |`true` for ONTAP 9.15.1 or later, otherwise `false`.
...
*Do not specify for ASA r2*.	|"0" if `snapshotPolicy` is "none", otherwise ""
...
*Do not specify for ASA r2*.|`unix`


|`tieringPolicy` |Tiering policy to use	"none" 

*Do not specify for ASA r2*.|"snapshot-only" for pre-ONTAP 9.5 SVM-DR configuration.
