...
+
----
Trident supports automatic configuration of backend configuration and creates the backend and storage classes seamlessly after Trident installation. To enable configuration of the backend and storage classes automatically, add the `ontapConfigurator` parameter and specify `authType`, `fsxnID`, `protocols` during the installation.
