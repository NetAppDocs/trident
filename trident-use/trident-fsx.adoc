...
If you intend to use `ontap-san` drivers (e.g. iSCSI), the required multipathing setup on the nodes may conflict with the Amazon Elastic Block Store (EBS) CSI driver. To ensure that multipathing works without impacting EBS disks on the same node, you must exclude EBS in your multipathing setup. The following is an example of a `multipath.conf` file that includes the required Trident settings and excludes EBS disks from multipathing:
