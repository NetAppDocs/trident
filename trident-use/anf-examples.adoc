...
keywords: trident backend, azure netapp files, smbvolumes, smb, windows, sub volume
...
|`filePoolVolumes` |List of parent volumes. 
...
*The `ro` mount option is not supported for the Azure NetApp Files sub volume driver or any other driver. If you specify `ro` for the Azure NetApp Files sub volume driver, backend creation will fail. If you specify `ro` mount option for any other driver, CSI operations will fail.*
...
----
{
    ...
    "subscriptionID": "9f87c765-4774-fake-ae98-a721add45451",
    "filePoolVolumes": ["/anf-resourcegroup/anf-account/ultra-pool/parent-volume-1"],
    "tenantID": "68e4f836-edc1-fake-bff9-b2d865ee56cf",
    "clientID": "dd043f63-bf8e-fake-8076-8de91e5713aa",
    "clientSecret": "SECRET",
    "location": "eastus"
...
[%collapsible%open]
====
This backend configuration places volumes in the Azure `eastus` location in an `Ultra` capacity pool. Astra Trident automatically discovers all of the subnets delegated to Azure NetApp Files in that location and places a new volume on one of them randomly.
