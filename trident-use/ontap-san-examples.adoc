...
You can configure Trident to authenticate to a backend SVM using Active Directory (AD) credentials. You must configure AD domain controller access to the cluster or SVM before an AD account can access the SVM. For cluster administration with an AD account, you must create domain tunnel. Refer to link:https://docs.netapp.com/us-en/ontap/authentication/enable-ad-users-groups-access-cluster-svm-task.html[Configure Active Directory domain controller access in ONTAP^] for details.
