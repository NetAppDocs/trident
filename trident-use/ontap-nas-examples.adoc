...
----
For snapshotReserve = 50%, and PVC request = 5GiB, the total volume size is 2/.5 = 10GiB and the available size is 5GiB, which is what the user requested in the PVC request. The `volume show` command shows results similar to this example:

image::../media/vol-show-nas.png[Shows the output of the volume show command.]  
