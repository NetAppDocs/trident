...
* *Kubernetes*:
** Added support for CSI Volume Group Snapshots with v1beta Volume Group Snapshot Kubernetes APIs, for iSCSI volumes.
** Added FSTRIM-support with LUKS-encrypted volumes, to reuse filesystem space properly. 
...
* Added NVMe/TCP protocol support for NetApp ASA (r2), in addition to iSCSI.

==== Experimental Enhancements:
* Added tech preview for Trident controller concurrency via the `--enable-concurrency` feature flag.
This feature is currently experimental and may not support all workflows. At this time, it is only enabled with the ONTAP-SAN driver (for iSCSI and FC protocols). 

==== Fixes

* *Kubernetes*:
** Fixed CSI NodeExpandVolume issue where multipath devices could be left with incongruent sizes if a SCSI disk was unavailable.
** Fixed failure to clean up duplicate export policies for ONTAP-NAS and ONTAP-NAS-Economy drivers.
** Fixed GCNV dual protocols defaulting to a single protocol when no NFS mount options were specified in GCNV backend.
