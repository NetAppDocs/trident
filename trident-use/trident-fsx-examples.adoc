...
|`apiRegion` |AWS API region name. Optional.|`""`
|`apikey` |AWS API key. Optional. |`""`
|`secretKey` |AWS secret key. Optional. |`""`

|`credentials` |For ONTAP drivers with FSxN personality, we must include credentials of type credential store (i.e., AWS secret manager).

- `name`: contains ARN of the secret (secret in turn will have credentials of SVM)
- `type`: awsarn |`""` 
