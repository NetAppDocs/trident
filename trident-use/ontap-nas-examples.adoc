...
NOTE: When the aggregate list is updated in ONTAP, the list is updated in Trident automatically by polling ONTAP without having to restart the Trident Controller. When you have configured a specific aggregate in Trident to provision volumes, if the aggregate is moved in ONTAP, the backend will move to failed state in Trident while polling ONTAP aggregate.
