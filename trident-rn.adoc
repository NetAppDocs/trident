...
* *Kubernetes*:
** Added support for CSI Volume Group Snapshots with v1beta1 Volume Group Snapshot Kubernetes APIs for ONTAP-NAS NFS driver. See link:https://docs.netapp.com/us-en/trident/trident-use/vol-group-snapshots.html[Work with volume group snapshots^].
** Enhanced qualification for vanilla KubeVirt for ONTAP drivers.
...
** Added ONTAP AFX support for ONTAP NAS driver. See link:https://docs.netapp.com/us-en/trident/trident-use/ontap-nas-examples.html[ONTAP NAS configuration options and examples^].
...
** Added option to serve Prometheus metrics with HTTPS. See link:../trident-use/monitor-trident.html[Monitor Trident^]
** Added option to keep original imported volume name but manage lifecycle with Trident. See link:../trident-use/vol-import.html[Import volumes^]
* *KubeVirt*: Enhanced qualification for vanilla KubeVirt for ONTAP drivers.
* Added option for Trident controller to use host networking via helm, operator and tridentctl (link:https://github.com/NetApp/trident/issues/858[Issue #858]).

==== Experimental Enhancements
NOTE: Not for use in production environments.

* *Tech Preview*: Added support for concurrency to ONTAP-SAN FC and ONTAP-NAS drivers.

==== Fixes

* *Kubernetes*:
** Fixed an issue where multiple attempts to close a LUKS device resulted in failures to detach volumes.
** Fixed CSI node-driver-registrar container name inconsistency by standardizing Linux DaemonSet to `node-driver-registrar` to match Windows DaemonSet and container image naming.
** Fixed Trident node pod not starting on Windows under Openshift due to SCC having `allowHostDirVolumePlugin` set to false (link:https://github.com/NetApp/trident/issues/950[Issue #950])
...
* Fixed inability to mount a Persistent Volume Claim (PVC) based on a snapshot of an XFS filesystem PVC on the same Kubernetes node.
