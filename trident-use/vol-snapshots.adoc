...
Astra Trident provides rapid, in-place volume restoration from a snapshot using the `TridentActionSnapshotRestore` CR. This CR functions as an imperative action and runs at the time of initiation only. 

Astra Trident supports snapshot restore on the `ontap-san`, `ontap-san-economy`, `ontap-nas`, `ontap-nas-flexgroup`, `azure-netapp-files`, `gcp-cvs`, and `solidfire-san` drivers. 

NOTE: Snapshot restore is not supported on qtree volumes. 
