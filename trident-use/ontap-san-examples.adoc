...
`igroupName` can be set to an igroup that is already created on the ONTAP cluster. We recommend using one igroup per Kubernetes cluster, if the SVM is to be shared between environments. This is necessary for Astra Trident to automatically maintain IQN additions and deletions.

The `igroupName` can be updated to point to a new igroup that is created and managed on the SVM outside of Astra Trident.
