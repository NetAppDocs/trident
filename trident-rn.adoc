...
* Added new flag `--k8s_api_qps` to installers to set the QPS value used by Trident to communicate with the Kubernetes API server.
Added `--node-prep` flag to installers for automatic management of storage protocol dependencies on Kubernetes cluster nodes.
* Added NFS support for Google Cloud Netapp Volumes driver.
* Added support for Cloud Identity.
** GKE with GCNV - GCP Workload identity will be used as Cloud Identity.
...
* Reduced Azure Netapp Files minimum volumen size to 50 GiB.
* Added `denyNewVolumePools` configuration paramet to restrict ONTAP-NAS-ECONOMY and ONTAP-SAN-ECONOMY drivers to preexisting Flexvol pools.
* Added tech preview Fibre Channel support on ONTAP-SAN driver.
...
* Kubernetes: Delete ephemeral snapshots created during cloning (link:https://github.com/NetApp/trident/issues/901[Issue #901]).
...
** Backend-scoped export policies will no longer be created when configuring ONTAP-NAS-Economy driver.
...
* Added ability for user-configured log settings for the Trident controller to be propagated to Astra Trident node pods.
* Added support in Astra Trident to use REST by default instead of ZAPI for ONTAP versions 9.15.1 and later.
...
* Added ability for Astra Trident DaemonSet to clean zombie mounts and residual tracking files at startup (link:https://github.com/NetApp/trident/issues/883[Issue #883]).
...
* Fixed Astra Trident installation failures due to stale transactions.
* Fixed tridentctl to ignore warning messages from Kubernetes (link:https://github.com/NetApp/trident/issues/892[Issue #892]).
* Changed Astra Trident controller `SecurityContextConstraint` priority to `0` (link:https://github.com/NetApp/trident/issues/887[Issue #887]).
* ONTAP drivers now accept volume sizes below 20MiB (link:https://github.com/NetApp/trident/issues/885[Issue[#885]).
* Fixed Astra Trident to prevent shrinking of Flexvols during resize operation for the ONTAP-SAN driver.
