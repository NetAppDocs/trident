...
NOTE: ##To define a VM-scoped Application that protects specific KubeVirt virtual machines (VMs) by name instead of entire namespaces, use the `spec.includedVirtualMachines` field in the Application CR or the `--virtual-machines` flag in the CLI. For details, field reference, and YAML examples, refer to link:trident-protect-kubevirt-vms.html[Protect KubeVirt virtual machines with Trident Protect].##
...
. (_Optional_) If needed, you can add resource filtering to the same CR to include or exclude specific resources:
+
