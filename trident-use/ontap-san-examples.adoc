...
|`formatOptions`
a| Use `formatOptions` to specify command line arguments for the `mkfs` command, which will be applied whenever the volume is formatted. This allows you to format the volume according to your preferences. Ensure that you specify the correct formatOptions with the `mkfs` command without the device path.
