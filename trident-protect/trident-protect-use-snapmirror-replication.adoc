...
. Optionally, on the source cluster, take a snapshot of the source application. This snapshot is used as the basis for the application on the destination cluster. If you skip this step, you'll need to wait for the next scheduled snapshot to run so that you have a recent snapshot. To create an on-demand snapshot, refer to link:https://docs.netapp.com/us-en/trident/trident-protect/trident-protect-protect-apps.html#create-an-on-demand-snapshot[Create an on-demand snapshot^].
. On the source cluster, create the replication schedule CR:
+
[NOTE]
====
...
+
NOTE: When using a CR, manually create the destination namespace before applying the CR. Trident Protect automatically creates namespaces only when using the CLI.
+
[role="tabbed-block"]
====
...
** *spec.storageClassName*: (_Optional_) Choose the name of a valid storage class on the cluster. The storage class must be linked to an ONTAP storage VM that is peered with the source environment. If the storage class is not provided, the default storage class on the cluster will be used by default.
** *spec.storageClassMapping*: (_Optional_) ##Maps source storage classes to destination storage classes for individual PVCs within the same AMR. Each entry requires a `source` and `destination` storage class name. When set, `storageClassMapping` overrides `storageClassName` entirely; PVCs without a matching source entry use the cluster default storage class, not `storageClassName`.##
+
##When using `storageClassMapping` in an AppMirrorRelationship CR, be aware of the following restrictions:##
+
* `storageClassMapping` overrides `storageClassName` entirely. When both fields are set, `storageClassName` is ignored.
* PVCs with source storage classes that have no matching entry in `storageClassMapping` use the cluster default storage class, not `storageClassName`.
* Each entry requires both a `source` and `destination` value.
* Changing the destination storage class can force deletion and recreation of existing destination PVCs.
** *spec.recurrenceRule*: Define a start date in UTC time and a recurrence interval.
+
Example YAML:
+
...
.. Create and apply the AppMirrorRelationship object, replacing values in brackets with information from your environment:
+
[source,console]
----
##tridentctl-protect create appmirrorrelationship <name_of_appmirorrelationship> --destination-app-vault <my_vault_name> --source-app-vault <my_vault_name> --recurrence-rule <rule> --namespace-mapping <ns_mapping> --source-app-id <source_app_UID> --source-app <my_source_app_name> --storage-class <storage_class_name> --storageclass-mapping  <src_class:dst_class> -n <application_namespace>##
----
+
NOTE: ##Use `--storageclass-mapping` to map individual source storage classes to destination storage classes. Specify one or more `source:destination` pairs separated by commas (for example, `"class-a:class-b,class-c:class-d"`). Use `*:destination-class` as a wildcard to map all unmapped PVCs to a single destination storage class. When `--storageclass-mapping` is set, `--storage-class` is ignored.##
+
##*Example using a single destination storage class (`--storage-class`):*##
+
[source,console]  
----
