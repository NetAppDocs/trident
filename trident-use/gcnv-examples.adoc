...
|`snapshotDir` |Access to the `.snapshot` directory |`true`, `false` (default behavior might vary. Set explicitly)
"false" for NFSv3
...
[%collapsible%closed]
====
When you install Trident with a GKE cloud identity, backends authenticate as a workload identity and omit the `apiKey` block. Cloud identity is configured the same way for both Google Cloud NetApp Volumes drivers. The only difference is the `storageDriverName` value: use `google-cloud-netapp-volumes` for NAS (NFS or SMB) volumes and `google-cloud-netapp-volumes-san` for SAN (iSCSI block) volumes.

NOTE: Cloud identity support for the `google-cloud-netapp-volumes-san` driver requires Trident 26.06 or later. SAN volumes are provisioned from Flex Unified storage pools, so set `serviceLevel` to `flex`.

.NAS driver (`google-cloud-netapp-volumes`)
[source,yaml]
----
apiVersion: trident.netapp.io/v1
kind: TridentBackendConfig
...
.SAN driver (`google-cloud-netapp-volumes-san`)
[source,yaml]
----
apiVersion: trident.netapp.io/v1
kind: TridentBackendConfig
metadata:
  name: backend-tbc-gcp-gcnv-san
spec:
  version: 1
