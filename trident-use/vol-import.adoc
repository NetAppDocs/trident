...
.Managed volume
--
The following example imports 1Gi ontap-nas volume `ontap_volume1` on backend `81abcb27-ea63-49bb-b606-0a5315ac5f21` using PVC annotations:
...
.Unmanaged volume
--
The following example imports 1Gi `ontap-nas` volume `<nas-volume2>` on backend `34abcb27-ea63-49bb-b606-0a5315ac5f34` as an unmanaged volume using PVC annotations:
