...
[.lead]
You can install Astra Trident using the Trident operator (manually or using Helm) or with `tridentctl`. This topic provides important information for selecting the right installation process for you. 
...
== Learn about Astra Trident installation options
To ensure Astra Trident can be installed in a wide variety of environments and organizations, NetApp offers multiple installation options. Before you get started, you need to make some decisions. 

* Select the appropriate installation _method_. Review <<Choose the right installation method>> to help make your decision. 
* Determine your deployment process based on the _installation mode_ required by your organization. 
+
// start tabbed area
...
|`tridentctl`
a|link:kubernetes-customize-deploy-tridentctl.html[Standard installation]

link:kubernetes-customize-deploy-tridentctl-mirror.html[Offline installation]
...
Whether deploying manually or using Helm, the Trident operator is a great way to dynamically manage Astra Trident resources and automate setup. You can even link:../trident-get-started/kubernetes-customize-deploy.html[customize your Trident operator deployment] using the attributes in the `TridentOrchestrator`. 

==== Benefits of using the Trident operator
