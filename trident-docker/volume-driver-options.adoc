...
== Cloud Volumes Service on Google Cloud volume options

You can control default volume provisioning using the following options. 
...
|`size`
a|The size of the volume. 

CVS-Performance service type defaults to 100 GiB minimum. 

CVS service type defaults to 1 GiB minimum. 

| `serviceLevel` |The CVS-Performance or CVS service level for new volumes. 

CVS-Performance values are `standard`, `premium`, or `extreme`. Default is "standard".

CVS values are `standardsw` or `zoneredundantstandardsw`. Default is "standardsw".
...
* Create a 2 TiB volume:
+
----
docker volume create -d netapp --name demo -o size=2T
----

* Create a 5 TiB premium volume:
+
----
