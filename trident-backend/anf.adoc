---
sidebar: sidebar
permalink: trident-backend/anf.html
...
The following `StorageClass` definitions refer to the storage pools above. By using the `parameters.selector` field, you can specify for each `StorageClass` the pool that is used to host a volume. The volume will have the aspects defined in the chosen pool.
