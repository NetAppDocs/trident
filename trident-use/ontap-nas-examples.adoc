...
Beginning with the 25.10 release, NetApp Trident supports link:https://docs.netapp.com/us-en/ontap-afx/index.html[ONTAP AFX storage systems^]. ONTAP AFX systems differ from other ONTAP systems (ASA, AFF, and FAS) in the implementation of their storage layer.

NOTE: Only the `ontap-nas` driver (with NFS protocol) is supported for ONTAP AFX systems; SMB protocol is not supported.

In the Trident backend configuration, you need not specify that your system is ONTAP AFX. When you select `ontap-nas` as the `storageDriverName`, Trident detects automatically the ONTAP AFX systems. Some backend configuration parameters are not applicable to ONTAP AFX systems as noted in the table below.
...
*Do not specify for ONTAP AFX storage systems.*
...
*Do not specify for ONTAP AFX storage systems.*|"" (not enforced by default)
...
*If specified, always set to `nfs` for ONTAP AFX storage systems*.| `nfs` 
...
*If specified, always set to `true` for ONTAP AFX storage systems*.
