...
*There are other options too which can save the formatting time:*
...
*-E:* Sets a flag in the filesystem superblock indicating that it may be mounted using experimental kernel.
...
*-g:* Specifies the number of blocks in a block group. There is generally no reason to ever set this parameter because the default is optimal for the filesystem.
