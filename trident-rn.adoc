...
* *Kubernetes*:
** Added support for CSI Volume Group Snapshots with v1beta Volume Group Snapshot Kubernetes APIs for ontap-san iSCSI driver.
** Added support for ONTAP ASA r2 for NVMe/TCP in addition to iSCSI.
** Added `--allow-discards` when opening LUKS volumes to enable filesystem space reclamation.
** Enhanced LUKS-encrypted volumes formatting performance.
...
* Added support to make FSGroup Policy configurable.
* Added secure SMB support for ONTAP-NAS and ONTAP-NAS-Economy volumes.
* Enhanced Trident node concurrency for iSCSI volumes.
* Enhanced Trident node concurrency for higher scalability on node operations for NFS and SMB volumes.
* Made split on clone operations asynchronous when creating volume clones.
* Added internalID field to the Trident volume config for ontap-nas-economy driver.
* Added option to add or override selectors via StorageClass annotations.
* Added support for volume replication for NVMe backends.

==== Experimental Enhancements:
NOTE: Not for use in production environments.

* [Tech Preview] Enabled concurrent Trident controller operations via the --enable-concurrency feature flag for ONTAP-SAN driver (iSCSI and FC protocols).
+
NOTE: This feature is experimental and does not support all workflows.
...
* *Kubernetes*:
** Fixed CSI NodeExpandVolume issue where multipath devices could be left with incorrect sizes when SCSI disk is unavailable.
** Fixed failure to clean up duplicate export policies for ONTAP-NAS and ONTAP-NAS-Economy drivers.
** Fixed GCNV volumes defaulting to NFSv3 only when nfsMountOptions is unset; now support both NFSv3 and NFSv4 protocols.
...
* Fixed issue where the ANF volume sizes are not automatically aligned to 1GiB increments.
