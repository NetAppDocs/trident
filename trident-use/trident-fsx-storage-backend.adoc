...
There are two way to configura the Trident backend to use an FSx for ONTAP file system:
* By creating a StorageClass that leverages the Automatic Backend feature.
* By creating the backend and StorageClass separately.

The steps in this section will cover how to create the backend separately. For information on how to use the Automatic Backend feature, refer to xref:trident-fsx-storageclass-pvc.html#configure-a-kubernetes-storageclass-object-with-automatic-backend-configuration-support-for-fsx-for-ontap[Configure StorageClass and PVC] section for more information.
