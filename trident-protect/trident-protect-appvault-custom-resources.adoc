...
* For S3 AppVault objects, you can optionally specify a sessionToken if you are using single sign-on (SSO) for authentication. This token is created when you generate keys for the provider in <<AppVault key generation examples for cloud providers>>.
=====
