...
*Do not specify for ASA r2*. <check for 25.06>
...
*Do not specify for ASA r2*.<check for 25.06>|"" (not enforced by default) 
...
*If specified, always set to `true` for ASA r2*. <check for 25.06> |`true` for ONTAP 9.15.1 or later, otherwise `false`.
...
*If specified, set to `true` for ASA r2*. <check for 25.06>

|`spaceReserve` |Space reservation mode; "none" (thin) or "volume" (thick).

*Set to `none` for ASA r2*. <check for 25.06> |"none"
...
*Do not specify for ASA r2*. <check for 25.06>	|"0" if `snapshotPolicy` is "none", otherwise ""
...
Set to `false` for ASA r2. <check for 25.06>
