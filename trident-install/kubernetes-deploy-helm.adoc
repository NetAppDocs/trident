...
Review link:kubernetes-deploy.html[the installation overview] to ensure you've met installation prerequisites and selected the correct installation option for your environment.

.Before you begin

In addition to the link:kubernetes-deploy.html#before-you-begin[deployment prerequisites] you need link:https://v3.helm.sh/[Helm version 3^].
...
. Use `helm install` and specify a name for your deployment as in the following example where `100..0` is the version of Trident you are installing. 
+
----
...
You can automate the force-detach process through integration with node health check (NHC) operator. For information, see link:../trident-reco/force-detach.html[Automating the failover of stateful applications with Trident].
|`false`
...
a| Enables concurrent Trident controller operations for improved throughput. For more information on supported drivers, see the link: https://docs.netapp.com/us-en/trident/trident-use/controller-scalability.html#controller-scalability[Controller Scalability documentation].|false
...
[NOTE]
=====
* No limits are applied by default - only requests have default values and are automatically applied if not specified.
...
|`hostNetwork` a|Enables host networking for the Trident controller. This is useful when you want to separate the frontend and backend traffic in a multi-home network. |false
