...
WARNING: Amazon FSx for NetApp ONTAP allows users to create backup schedules. Volumes created by Trident can be backed up daily, within a backup window that you define. If you choose to use backup policies, volumes must be cleaned up through FSx. Users will need to delete their PVs manually and then proceed to delete them on the FSx plane.
