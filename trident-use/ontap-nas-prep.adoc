...
Beginning with the 25.06 release, NetAPP Trident supports seucure provisioning of SMB volumes created using `ontap-nas` and `ontap-nas-economy` backends. When secure SMB is enabled, you can provide controlled access to SMB shares for Active Directory (AD) users using Access Acontrol Lists (ACLs).

.Points to remember

* Volume import is not supported with secure SMB.
* Only read-only clones are supported when secure SMB is enabled.
