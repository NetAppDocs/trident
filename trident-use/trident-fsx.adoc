...
WARNING: Volumes created on Amazon FSx for NetApp ONTAP will need to be manually deleted. This is done by deleting the PersistentVolume and then deleting the backing volume using the FSx console. Since each FSx volume comes preconfigured with a SnapMirror, Trident cannot modify or break it.
