...
|`cloudIdentity` | Set to `azure.workload.identity/client-id: xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxx` when using cloud identity on an AKS cluster. |`""`
...
[%collapsible%closed]
====
This example installs Astra Trident to use a cloud identity on an AKS cluster. 
