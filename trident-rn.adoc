...
* Added a `SkipApplicationCreation` field on Restore CRs and implemented conditional Application CR creation.
* Added resource transformations support to all Restore CRs.
* Made the temporary-volume `splitOnClone` annotation configurable for Kopia backups.
...
* Fixed cross-cluster VM restore that failed with a webhook error when the source namespace did not exist on the target cluster.
* Fixed selective VM in-place restore that failed at CreateApplication because of a spec mismatch on an existing Application. Trident Protect now reuses the existing Application.
* Fixed an in-place restore (IPR) job that hung in the Running state when both the VM and its PVC were deleted. A deleted PVC during an IPR is now treated as a fatal error.
* Fixed an immediate backup or snapshot that was triggered when editing an existing schedule by correcting the scheduled timestamp for overdue runs.
* Fixed the Snapshot finalizer that attempted to fetch the Application CR after it was deleted. The finalizer now skips when the Application is not found.
* Fixed protected capacity increasing significantly after an IPR by using a sparse copy.
* Fixed Trident Protect deleting the Kubernetes snapshot object before confirming ONTAP deletion. The finalizer now waits for VolumeSnapshot deletion.
* Fixed snapshots that did not continue when a VM freeze failed. Freeze and unfreeze now continue on failure.
* Fixed KubeVirt VM applications that recreated PVCs during AMR reverse, reverse-resync, and resync operations.
