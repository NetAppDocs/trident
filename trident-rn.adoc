...
** Fixed a race between CSI publish and volume move that could attach a volume using stale access information.
** Fixed a race where NodeGetInfo could return empty topology labels before node registration completed, causing topology-aware provisioning failures.
** Fixed a rare NVMe disconnect race that could tear down a shared subsystem while another pod was still attaching.
** Fixed NVMe PVC clone publish failures (and incorrect parent-namespace mapping) when creating clones of a mounted volume under concurrency.
** Fixed snapshot creation failures for PVCs on ONTAP-SAN-Economy backends using a nameTemplate.
...
** Added general availability (GA) support for concurrency for the ONTAP-NAS-Economy and ONTAP-SAN-Economy drivers, graduating this functionality from Tech Preview, and added GA concurrency support for the Azure NetApp Files (ANF) and SolidFire drivers.
** Added support for Kubernetes 1.36.
** Added general availability (GA) support for volume autogrow with the Google Cloud NetApp Volumes (GCNV) NAS and SAN drivers. 
** Added Unified ONTAP balanced placement for optimal aggregate selection when provisioning new volumes (link:https://github.com/NetApp/trident/issues/64[Issue #64]).
** Added ONTAP AFX support for the ONTAP-NAS-Flexgroup driver.
** Added integration with the NetApp Shift Toolkit to simplify virtual machine migrations when using the Red Hat OpenShift Migration Toolkit for Virtualization.
** Added Cloud Identity support for the GCNV SAN driver.
** Added subordinate volume support for Trident Volume Publication (TVP) propagation and autogrow workflows.
** Added support for volume updates and volume group snapshots when running Trident with concurrency enabled.
** Added dynamic gRPC rate limiting for GCNV.
** Added a controller readiness exec probe and tuned the default Kubernetes API QPS and Burst settings for the controller and sidecars.
** Improved node operation sequencing by decoupling the attach, encryption setup, and filesystem formatting steps.
** Mounted Trident container root filesystems as read-only to improve security.
** Added best-effort snapshot removal for clone volume snapshots in ONTAP backends when the source snapshot of a clone volume is deleted.
** Added backup clone PVC recognition to prevent temporary PVCs created by Veeam Kasten, Trilio, and Cohesity from becoming stuck in the ONTAP Volume Recovery Queue.
** Enhanced AWS ARN handling to support special regions.
** Prevent shrinking of Flexvols during resize operation for the ONTAP-SAN-ECONOMY driver.
...
* *[Tech Preview]*: Added support for concurrency for the ASA-R2 (SAN and NVMe) drivers.
* *[Tech Preview]*: Added GCNV ONTAP-mode HTTP transport for the ONTAP-SAN and ONTAP-NAS drivers.

==== Fixes

* *Kubernetes*:

** Fixed race conditions and state handling for concurrent publish/unpublish, clone, cache, and backend update operations.
** Fixed export policy race conditions and concurrent publish/unpublish handling for subordinate volumes and read-only clones in the ONTAP-NAS and ONTAP-NAS-Economy drivers.
** Fixed ONTAP-NAS-Economy FlexVol discovery after a MetroCluster failover when snapshot policies differ (link:https://github.com/NetApp/trident/issues/1082[Issue #1082]).
** Fixed ONTAP-NAS-Economy volume delete retries that interrupted long-running qtree deletes (link:https://github.com/NetApp/trident/issues/1121[Issue #1121]).
** Fixed ONTAP-SAN and ONTAP-SAN-Economy import and resize behavior, including fsType validation, volume metadata handling, and autogrow mode behavior.
** Fixed an NVMe/TCP namespace race condition during concurrent volume creation (link:https://github.com/NetApp/trident/issues/1089[Issue #1089]).
** Fixed iSCSI resize failures that occurred when available paths and portals did not match, and improved repeated resize behavior.
** Fixed ONTAP-SAN volume delete failures caused by volume busy conditions.
** Fixed ONTAP REST error handling and improved SVM state timeout behavior.
** Fixed GCNV SAN zonal pool and host group handling, normalized volume identifier conversion, and normalized create requests below the minimum size.
** Fixed native GCNV capacity pool selection to exclude ONTAP-mode pools.
** Fixed KubeVirt failover for VMs that use Trident Automated Workload Failover to speed up VM failover.
** Fixed Operator-managed installation RBAC issues, including missing node ClusterRoles and incorrect ClusterRole and ClusterRoleBinding reconciliation during upgrades. This applies to Operator and Helm-based installations that use the Trident Operator chart.
** Fixed persistent GitOps diffs on the TridentConfigurator and related CRDs by omitting default priority values and correcting YAML template indentation (link:https://github.com/NetApp/trident/issues/915[Issue #915], link:https://github.com/NetApp/trident/issues/1120[Issue #1120]).
** Fixed patching of TridentVolumePublication CRDs during a Helm upgrade (link:https://github.com/NetApp/trident/issues/1151[Issue #1151]).
** Fixed CSI startup ordering by starting the CSI gRPC service before node registration retries.
** Fixed secure SMB clone configuration overlays during concurrent operations.
** Fixed missing annotations on the Trident controller pod after an upgrade (link:https://github.com/NetApp/trident/issues/1004[Issue #1004]).
** Improved slow qtree provisioning on large SVMs (link:https://github.com/NetApp/trident/issues/1142[Issue #1142]).
** Fixed ControllerPublish to use the volume configuration filesystem type instead of an incorrect default filesystem type.
** Fixed an issue where orphan dm devices could cause repeated CSI NodeUnstageVolume failures.
** Improved stale node and multipath cleanup by removing ghost multipath artifacts and stale TridentNode resources after soft-deleted node cleanup.

=== Trident Protect

==== Enhancements

* Added OpenShift Service CA Operator integration to automate TLS certificate management for the Trident Protect admission webhook. Enable by setting `useOpenshiftServiceCA` to `true` in your Helm values. To learn more, refer to link:trident-protect/trident-protect-customize-installation.html[Customize Trident Protect installation^].
* Added support for backing up and restoring KubeVirt VMs on OpenShift Virtualization. Protect all VMs in a namespace or target specific VMs by name, and selectively restore individual VMs from a backup or snapshot. To learn more, refer to link:trident-protect/trident-protect-kubevirt-vms.html[Protect KubeVirt virtual machines with Trident Protect^].
* Added support for resource transformations, enabling you to add, remove, and modify Kubernetes resource attributes during the restore process. To learn more, refer to link:trident-protect/trident-protect-resource-transformations.html[Modify resources as you restore them using custom resources].
* Added support for `storageClassMapping` in AppMirrorRelationship (AMR), enabling storage class mapping for individual PVCs within a single AMR definition. To learn more, refer to link:trident-protect/trident-protect-use-snapmirror-replication.html[Replicate applications using NetApp SnapMirror and Trident Protect^].

==== Fixes

* Fixed cross-cluster virtual machine restores failing with an admission webhook error when the source application namespace did not exist on the destination cluster.
* Fixed selective virtual machine in-place restores failing at the application creation step due to a specification mismatch with an existing application custom resource; the existing application is now reused.
* Fixed virtual machine in-place restore jobs hanging in a `Running` state when both the virtual machine and its PersistentVolumeClaim were deleted; a missing PersistentVolumeClaim is now treated as a fatal error.
* Fixed editing an existing protection schedule incorrectly triggering an immediate backup or snapshot run.
* Fixed the snapshot finalizer attempting to fetch the application custom resource after it was deleted; the finalizer now skips when the application is not found.
* Fixed protected storage capacity increasing significantly after a virtual machine in-place restore by using sparse copy during the PersistentVolumeClaim copy step.
* Fixed Trident Protect deleting the Kubernetes snapshot object before confirming deletion of the backing snapshot in ONTAP; the finalizer now waits for `VolumeSnapshot` deletion.
* Fixed snapshots not proceeding when a virtual machine freeze operation failed; freeze and unfreeze operations now continue on failure.
* Fixed KubeVirt virtual machine applications incorrectly recreating PersistentVolumeClaims during AppMirrorRelationship reverse, reverse resync, and resync operations.

== What's new in 26.02.1
Learn what's new in NetApp Trident and Trident Protect, including enhancements, fixes, and deprecations.
...
* Fixed Google Cloud NetApp Volumes (GCNV) SAN behavior to support Flex Unified ZONAL pools for block (iSCSI) workloads. Both zonal and regional configurations are now supported.
* Fixed Google Cloud NetApp Volumes (GCNV) NAS UNIFIED pool handling by improving volume creation, validation, naming, and fallback volume lookup operations when `InternalID` is absent.
* Fixed an issue where iSCSI device resize operations could fail when available paths and portals did not match.
* Fixed an intermittent issue where `blkid` incorrectly identified the filesystem type for LUKS devices, preventing new volume creation.
* Fixed an issue where LUKS publish and stage operations at scale could block the Trident controller. Users are now expected to track LUKS passphrases associated with volumes.
* Fixed an issue where iSCSI multipath partitions could appear as ghost devices, preventing CSI `NodeUnstage` operations from succeeding.
* Fixed an NVMe/TCP namespace race condition during concurrent volume creation (Issue #1089).
* Fixed concurrent clone snapshot operations when snapshots were executed within the same second.
* Fixed an issue preventing KubeVirt VMs using `dataVolumes` from failing over with Trident Automated Workload Failover.
* Fixed an error that occurred when deleting an invalid `TridentMirrorRelationship` custom resource while using the Trident concurrent core.
* Fixed indentation in the `tridentactionmirrorupdates` Kubernetes CRD YAML definition (Issue #1120).
* Fixed the REST qtree asynchronous delete API call (Issue #1121).
* Fixed `ControllerPublish` to use the volume configuration filesystem type instead of an incorrect default filesystem type.
* Fixed controller startup logging to correctly report concurrent driver status.
...
==== Fixes
Updated Trident Protect dependencies to resolve security vulnerabilities:
...
* Go runtime: link:https://nvd.nist.gov/vuln/detail/CVE-2025-22871[CVE-2025-22871^]
* Additional dependencies: link:https://nvd.nist.gov/vuln/detail/CVE-2025-23109[CVE-2025-23109^]
...
** Added general availability (GA) support for concurrency for ONTAP-NAS (NFS only), ONTAP-SAN (iSCSI, FCP, NVMe), and Google Cloud NetApp Volumes (GCNV) drivers, graduating this functionality from Tech Preview. See the link:https://docs.netapp.com/us-en/trident/trident-use/controller-scalability.html[Controller scalability documentation] for more information.
** Added support for volume autogrow in Trident based on custom, user-defined Trident AutoGrow policies. See the link:https://docs.netapp.com/us-en/trident/trident-use/automatic-volume-expansion.html[Automatic volume expansion documentation] for more information.
** Enhanced Trident node concurrency to improve the scalability of node operations for NVMe volumes. See the link:https://docs.netapp.com/us-en/trident/trident-use/trident-nvme-rwx-subsystem-limits.html[NVMe volumes documentation] for more information.
** Added support for GCNV NAS volume auto-tiering through PVC annotations (`tieringPolicy`, `tieringMinimumCoolingDays`), including pool selection and clone inheritance. See the link:https://docs.netapp.com/us-en/trident/trident-use/gcnv-auto-tiering.html[Configure auto-tiering for Google Cloud NetApp Volumes documentation] for more information.
** Added support for GCNV SAN block (iSCSI) volumes using the `google-cloud-netapp-volumes-san` driver, including provisioning, per-node host group mapping for LUN access, and clone-from-volume operations. See the link:https://docs.netapp.com/us-en/trident/trident-use/gcnv-block.html[Google Cloud NetApp Volumes block configuration documentation] for more information.
** Added support for automatic backend configuration for Amazon FSx for NetApp ONTAP. When you create a StorageClass that includes the required parameters, Trident automatically creates the corresponding backend and VolumeSnapshotClass, if needed. See the link:https://docs.netapp.com/us-en/trident/trident-use/trident-fsx-auto-backend-configuration.html#add-additional-fsxn-file-systems[Amazon FSx for NetApp ONTAP backend configuration documentation] for more information.
** Added support for different Microsoft Azure clouds, such as Azure Government and Azure China, and custom cloud configuration for Azure NetApp Files backends. See the link:https://docs.netapp.com/us-en/trident/trident-use/anf.html[Azure NetApp Files backend configuration documentation] for more information.
** Added support for Kubernetes 1.35. See the link:https://docs.netapp.com/us-en/trident/trident-get-started/requirements.html[Requirements documentation] for more information.
...
[WARNING]
====
There is a known issue with the `csi-snapshotter` sidecar. In all Kubernetes versions, VolumeGroupSnapshots `v1beta1` prevents VolumeSnapshots from reaching the `ReadyToUse` state.
====

Two workarounds are available:

. Delete the VolumeGroupSnapshots CRDs to disable VolumeGroupSnapshots, and then reinstall Trident.

. Install VolumeGroupSnapshots `v1beta2` and `snapshot-controller` version 8.4.0 or later, and then reinstall Trident. VolumeGroupSnapshots do not function on Kubernetes versions earlier than v1.34.

==== Fixes

* *Kubernetes*:

** Fixed an issue where unpublishing a read-only clone removed export policy rules from the source volume in ONTAP-NAS, ONTAP-NAS-Economy, and Google Cloud NetApp Volumes drivers link:https://github.com/NetApp/trident/issues/1086[Issue #1086].
** Switched kubectl images to lightweight Alpine-based variants to prevent pull failures following Bitnami's public image deprecation https://github.com/NetApp/trident/issues/1080[Issue #1080].
** Fixed preserving annotation of existing deployment during Trident upgrade link:https://github.com/NetApp/trident/issues/1004[Issue #1004].
** Allow clone across different storage classes if both storage classes are pointing to the same backend link:https://github.com/NetApp/trident/issues/1104[Issue #1104].
...
** Improved CSI and ONTAP clone split timeouts to avoid race conditions with some backup applications link:https://github.com/NetApp/trident/issues/1098[Issue #1098], link:https://github.com/NetApp/trident/issues/1100[Issue #1100].
** Fixed suppression of LUKS error messages link:https://github.com/NetApp/trident/issues/1069[Issue #1069].
...
* Updated the OpenTelemetry-Go package to fix link:https://nvd.nist.gov/vuln/detail/cve-2026-24051[CVE-2026-24051].

=== Trident Protect

==== Enhancements

* Trident Protect now automatically disables protection schedules and cancels in-progress operations before an in-place restore, and re-enables them after the restore completes. To learn more, refer to link:trident-protect/trident-protect-restore-apps.html[Restore applications using Trident Protect^].
* Added the `runImmediately` field to the schedule CR and `--run-immediately` CLI flag to trigger an immediate backup or snapshot upon schedule creation. To learn more, refer to link:trident-protect/trident-protect-protect-apps.html#create-a-data-protection-schedule[Create a data protection schedule^].
* Added support for specifying a custom name for the restored application using the `destinationApplicationName` field in the restore CR or the `--destination-app-name` CLI flag. To learn more, refer to link:trident-protect/trident-protect-restore-apps.html[Restore applications using Trident Protect^].
...
* *Kubernetes*:
** Added support for CSI Volume Group Snapshots with v1beta1 Volume Group Snapshot Kubernetes APIs for ONTAP-NAS NFS and ONTAP-SAN-Economy drivers, in addition to ONTAP-SAN (iSCSI and FC). See link:https://docs.netapp.com/us-en/trident/trident-use/vol-group-snapshots.html[Work with volume group snapshots^].
** Added support for automated workload failover with force volume detach for the ONTAP-NAS and ONTAP-NAS-Economy (excluding SMB in both the NAS drivers), and the ONTAP-SAN and ONTAP-SAN-Economy drivers. See link:https://docs.netapp.com/us-en/trident/trident-reco/force-detach.html[Automating the failover of stateful applications with Trident^].
...
** Added an option to serve Prometheus metrics with HTTPS, instead of HTTP. See link:https://docs.netapp.com/us-en/trident/trident-use/monitor-trident.html[Monitor Trident^].
...
+
See link:https://docs.netapp.com/us-en/trident/trident-protect/trident-protect-protect-apps.html[Supported backup and schedule annotations^].
* Added an annotation to the schedule CR to configure PVC bind timeout, which will be used by the backup CR: `protect.trident.netapp.io/pvc-bind-timeout-sec`. See link:https://docs.netapp.com/us-en/trident/trident-protect/trident-protect-protect-apps.html[Supported backup and schedule annotations^].
...
IMPORTANT: For customers using SolidFire, do not upgrade to 25.06.1 due to a known issue when unpublishing volumes. 25.06.2 will be released soon to address this issue.
...
+
** Added support for ONTAP ASA r2 for NVMe/TCP in addition to iSCSI. See link:https://docs.netapp.com/us-en/trident/trident-use/ontap-san-examples.html[ONTAP SAN configuration options and examples^].
...
** Fixed GCNV volumes defaulting to NFSv3 when `nfsMountOptions` is unset; now both NFSv3 and NFSv4 protocols are supported. If `nfsMountOptions` is not provided, the host’s default NFS version (NFSv3 or NFSv4) are used.
