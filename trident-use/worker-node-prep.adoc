...
.NFS tools
link:https://docs.netapp.com/us-en/trident/trident-use/worker-node-prep.html#nfs-volumes[Install the NFS tools] if you are using: `ontap-nas`, `ontap-nas-economy`, `ontap-nas-flexgroup`, or `azure-netapp-files`.
...
.SMB tools
Refer to link:https://docs.netapp.com/us-en/trident/trident-use/worker-node-prep.html#prepare-to-provision-smb-volumes[Prepare to provision SMB volumes] if you are using: `ontap-nas` to provision SMB volumes.
