...
Before you can provision volumes using Trident, you need to integrate your storage backend with Trident by creating a Trident Backend Configuration (TBC). The TBC defines how Trident connects to the storage backend and provides the necessary information for Trident to manage the storage resources.

== Create a Trident Backend Configuration (TBC) for FSx for ONTAP
There are two ways to configure the Trident backend to use an FSx for ONTAP file system:
* Create a StorageClass that leverages the Automatic Backend feature.
* Create the backend and StorageClass separately.

The steps in this section cover how to create the backend separately. For information on how to use the Automatic Backend feature, refer to the link:trident-fsx-storageclass-pvc.html#configure-a-storage-class-without-automatic-backend-configuration[Configure StorageClass and PVC] section for more information.
...
*ONTAP NAS drivers*: NetApp recommends specifying dataLIF. If not provided, Trident fetches dataLIFs from the SVM. You can specify a fully-qualified domain name (FQDN) to be used for the NFS mount operations, allowing you to create a round-robin DNS to load-balance across multiple dataLIFs. Can be changed after initial setting. Refer to https://kb.netapp.com/Cloud/Astra/Trident/Is_it_possible_to_update_the_DataLIF_on_the_trident_backend[Update dataLIF after initial configuration^]. 
...
NOTE: Backend configuration parameters that affect backend identity and volume naming, such as `storageDriverName` and `storagePrefix`, cannot be changed after creation. To update them, you must create a new backend.
...
|`luksEncryption` 
| Enable LUKS encryption. Refer to link:../trident-reco/security-luks.html[Use Linux Unified Key Setup (LUKS)]. 
...
Before you complete link:trident-fsx-storage-backend.html#fsx-for-ontap-driver-details[ONTAP SAN and NAS driver integration] complete these steps: link:worker-node-prep.html#prepare-to-provision-smb-volumes[Prepare to provision SMB volumes].
