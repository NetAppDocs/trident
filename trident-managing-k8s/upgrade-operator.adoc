...
`bundle_pre_1_25.yaml` or `bundle_post_1_25.yaml` based on your Kubernetes version. For example, if you are installing Trident 25.02, run the following command:
+
----
...
.Steps
. If you link:../trident-get-started/kubernetes-deploy-helm.html#deploy-the-trident-operator-and-install-trident-using-helm[installed Trident using Helm], you can use `helm upgrade trident netapp-trident/trident-operator --version 100.2502.0` to upgrade in one step. If you did not add the Helm repo or cannot use it to upgrade:
...
.. Use the `helm upgrade` command where `trident-operator-25.02.0.tgz` reflects the version that you want to upgrade to.
+
----
