...
== Known limitations
You must configure both NFS and SMB/CIFS protocols on the SVM to create an `ontap-nas`, `ontap-nas-economy`, or `ontap-nas-flexgroup` volume. Failure to configure both will cause volume creation to fail.