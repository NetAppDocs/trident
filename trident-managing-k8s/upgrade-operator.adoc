...
[.lead]
You can upgrade Trident using the Trident operator either manually or using Helm. You can upgrade from a Trident operator installation to another Trident operator installation or upgrade from a `tridentctl` installation to a Trident operator version. Review link:upgrade-trident.html#step-3-select-an-upgrade-method[Select an upgrade method] before upgrading a Trident operator installation.  
...
.Steps
. If you link:../trident-install/kubernetes-deploy-helm.html#deploy-the-trident-operator-and-install-trident-using-helm[installed Trident using Helm], you can use `helm upgrade trident netapp-trident/trident-operator --version 100.2602.0` to upgrade in one step. If you did not add the Helm repo or cannot use it to upgrade:
...
.. Use the `helm upgrade` command where `trident-operator-26.06.0.tgz` reflects the version that you want to upgrade to.
+
----
helm upgrade <name> trident-operator-26.06.0.tgz
----
...
NOTE: Before switching between installation methods, review link:../trident-install/kubernetes-deploy.html#moving-between-installation-methods[Moving between installation methods].

.Steps
. Download the latest Trident release.
+
----
# Download the release required [26.06.0]
mkdir 26.06.0
cd 26.06.0
