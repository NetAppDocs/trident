...
For information, see link:../trident-reco/force-detach.html[Automating the failover of stateful applications with Trident].|`false`
...
|`tridentImage` |Trident image to install |`"netapp/trident:25.10"`
...
NOTE: *Tech Preview*: This feature is experimental and currently supports limited parallel workflows with the ONTAP-NAS (NFS only) and ONTAP-SAN (NVMe for unified ONTAP 9) drivers, in addition to the existing tech preview for the ONTAP-SAN driver (iSCSI and FCP protocols in unified ONTAP 9).|false

|`resources` a|Sets Kubernetes resource limits and requests for the Trident controller and node pods. You can configure CPU and memory for each container and sidecar to manage resource allocation in Kubernetes.
