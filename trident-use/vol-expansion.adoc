...
summary: Trident provides Kubernetes users the ability to expand their volumes after they are created. Find information about the configurations required to expand iSCSI and NFS volumes.
---
...
Trident 19.10 introduces support for expanding an iSCSI Persistent Volume (PV) by using the CSI provisioner.
...
=== Configure the StorageClass to support volume expansion
...
=== Create a PVC with the StorageClass you created
...
=== Define a pod that attaches the PVC
...
Starting with version 18.10, Trident supports volume expansion for NFS PVs provisioned on `ontap-nas`, `ontap-nas-economy`, `ontap-nas-flexgroup`, `aws-cvs`, `gcp-cvs`, and `azure-netapp-files` backends.

NOTE: Volume resize was introduced in Kubernetes v1.8 as an alpha feature and was promoted to beta in v1.11, which means this feature is enabled by default starting with Kubernetes v1.11.
